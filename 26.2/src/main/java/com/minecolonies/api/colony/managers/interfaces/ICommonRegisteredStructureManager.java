...
public interface ICommonRegisteredStructureManager<B extends ICommonBuilding,T>
{
    ...
    /**
     * Get the building nearest a position out of those matching the conditions.
     * <p>
     * 26.2/Fabric: {@link #getFirstBuildingMatching(Predicate)} returns whichever building the map happens to yield
     * first, which is position-hash order and has nothing to do with where the asker is. That is harmless when at most
     * one building can match, and wrong the moment two can -- a colony with several taverns sent every homeless citizen
     * in the town to the same one. Anything picking "the" building of a kind that a colony may have more than one of
     * should ask for the nearest instead, anchored the way the housing rule anchors
     * ({@link IRegisteredStructureManager#getHousingAnchor(com.minecolonies.api.colony.ICitizenData)}), so the
     * automatic paths and the commands speak with one voice.
     *
     * @param anchor    the position to measure from.
     * @param predicate the predicate matching the building.
     * @return the nearest matching building, or null if none matches.
     */
    @Nullable
    default B getClosestBuildingMatching(final BlockPos anchor, final Predicate<B> predicate)
    {
        B best = null;
        ...
        for (final B building : getBuildings().values())
        {
            ...
            // Ties broken by position so that two equidistant buildings do not swap between calls with map order.
            if (best == null
                  || distance < bestDistance
                  || (distance == bestDistance && BlockPosUtil.POSITION_ORDER.compare(building.getPosition(), best.getPosition()) < 0))
            {
                best = building;
