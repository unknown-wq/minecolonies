...
public class RequestTag implements IDeliverable
{
    ...
    public static CompoundTag serialize(@NotNull final HolderLookup.Provider provider, final IFactoryController controller, final RequestTag input)
    {
        ...
        {
            compound.put(NBT_RESULT, Utils.serializeCodecMess(ItemStack.OPTIONAL_CODEC, provider, input.getResult()));
        }
    ...
    {
        final TagKey<Item> theTag = TagKey.create(Registries.ITEM, buffer.readIdentifier());
    ...
    {
        final TagKey<Item> theTag = TagKey.create(Registries.ITEM, Identifier.parse(compound.getStringOr(NBT_TAG, "")));
