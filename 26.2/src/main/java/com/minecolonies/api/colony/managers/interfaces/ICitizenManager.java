...
public interface ICitizenManager extends IEntityManager
{
    ...
    /**
     * Get all citizens without copying them first. Use this for plain read-only iteration; it is the same data as
     * {@link #getCitizens()} but without the defensive ArrayList, which matters on the paths that run every tick.
     * <p>
     * The returned collection is a live, unmodifiable view of the backing map: do NOT hold on to it across a call
     * that can add or remove a citizen, and do not iterate it while doing so - use {@link #getCitizens()} there.
     *
     * @return an unmodifiable view of the citizens.
     */
    Collection<ICitizenData> getCitizensUnmodifiable();
     ...
     *
     * @return the random citizen, or null if the colony has none.
     */
    @Nullable
