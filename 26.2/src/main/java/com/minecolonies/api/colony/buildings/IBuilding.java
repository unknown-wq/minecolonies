...
public interface IBuilding extends IBuildingContainer, IBuildingModuleContainer, IRequestResolverProvider, IRequester, ICommonBuilding
{
    ...
    /**
     * Requests a build that ends at a given level rather than at the next one, skipping the levels in between.
     * <p>
     * Free mode only; without it this falls back to the ordinary one step upgrade.
     *
     * @param player      the requesting player.
     * @param builder     the assigned builder.
    ...
    /**
     * Requests a repair for the current building, saying what came of it.
     * <p>
     * Same path as {@link #requestRepair(BlockPos)} - that one is this one with {@code announce} true. The command
     * that repairs a whole colony is the caller that wants it quiet: one chat line per hut is unreadable at sixty
     * buildings, and it needs the outcome as a value it can count anyway.
     *
     * @param builder  the assigned builder, {@link BlockPos#ZERO} to let any builder take it.
     * @param announce whether to tell the colony's players about the new order, or about why there is none.
     * @return what came of it.
     */
    WorkOrderRequestResult requestRepair(BlockPos builder, boolean announce);
