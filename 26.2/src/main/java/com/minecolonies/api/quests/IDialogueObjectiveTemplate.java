...
public interface IDialogueObjectiveTemplate extends IQuestObjectiveTemplate
{
    ...
    class AnswerElement
    {
        ...
        public static AnswerElement parse(final JsonObject jsonObject)
        {
            ...
            return new AnswerElement(Component.translatableEscape(jsonObject.get(ANSWER_ID).getAsString()), IMinecoloniesAPI.getInstance().getQuestDialogueAnswerRegistry().getValue(Identifier.parse(resultObj.get(TYPE_ID).getAsString())).produce(resultObj));
        }
    }
}
