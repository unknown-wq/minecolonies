...
public interface ICitizenHappinessHandler
{
    ...
    /**
     * Drop the cached happiness value so that it is recomputed on the next query.
     * <p>
     * Happiness is only recomputed once per day (or when a modifier is reset), so anything that changes an input of a happiness modifier outside of that rhythm
     * (e.g. altering saturation, which feeds the social modifier) has to invalidate the cache explicitly, or the citizen -- and the colony overall happiness
     * derived from it, and the client side view serialized from it -- keeps reporting a stale value.
     */
    void invalidateCachedHappiness();
