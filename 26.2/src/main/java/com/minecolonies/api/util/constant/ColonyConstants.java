...
public final class ColonyConstants
{
     ...
     */
    // 26.2: TicketType became `record TicketType(long timeout, int flags)` — no type parameter, no comparator,
    // and TicketType#register is private, so the mod builds the record directly.
    public static final TicketType KEEP_LOADED_TYPE = new TicketType(TicketType.NO_TIMEOUT, TicketType.FLAG_LOADING | TicketType.FLAG_SIMULATION);
