        ...
        for (final CreativeModeTab tab : CreativeModeTabs.allTabs())
        {
            if (tab.getType() == CreativeModeTab.Type.CATEGORY)
            {
                ...
                if (tab.getDisplayItems().isEmpty())
                {
                    ...
                    try
                    {
                        // 26.2/Fabric: NeoForge's EventHooks.onCreativeModeTabBuildContents does not exist.
                        // The generator can be invoked directly through the (access-widened) displayItemsGenerator
                        // field with an ad-hoc Output collector.
                        tab.displayItemsGenerator.accept(displayParams, (stack, vis) -> stacks.add(stack));
                    }
