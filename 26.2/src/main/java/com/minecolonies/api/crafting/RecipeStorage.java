...
public class RecipeStorage implements IRecipeStorage
{
    ...
    private void processInputsAndTools(@Nullable final List<ItemStack> secOutputs)
    {
        ...
        for (final ItemStorage input : input)
        {
            ...
            final ItemStack container = craftingRemainderOf(inputItem.getItemStack());
    ...
    private boolean canFulfillItemStorage(final int qty, final Map<ItemStorage, Integer> existingRequirements, int availableCount, final ItemStorage storage)
    {
        ...
        {
            final ItemStack container = craftingRemainderOf(stack);
    ...
    private boolean checkForFreeSpace(final List<IItemHandler> handlers)
    {
        ...
        else
        {
            ...
            {
                final ItemStack container = craftingRemainderOf(stack.getItemStack());
     ...
     */
    /**
     * 26.2 removed {@code ItemStack#getCraftingRemainingItem()}; the remainder lives on the item as an
     * {@code ItemStackTemplate} (/opt/mc-src/net/minecraft/world/item/Item.java:284).
     *
     * @param stack the stack being consumed.
     * @return the container/remainder stack, or empty.
     */
    private static ItemStack craftingRemainderOf(final ItemStack stack)
    {
        final net.minecraft.world.item.ItemStackTemplate remainder = stack.getItem().getCraftingRemainder();
    ...
    public List<ItemStack> fullfillRecipeAndCopy(final LootParams context, final List<IItemHandler> handlers, boolean doInsert)
    {
        ...
        final AbstractEntityCitizen citizen = (AbstractEntityCitizen) context.contextMap().getOptional(LootContextParams.THIS_ENTITY);
                    ...
                    if(citizen != null && isTool)
                    {
                        if (stack.isDamageableItem())
                        {
                            ...
                            if (!ItemStackUtils.isEmpty(toDamage))
                            {
                                ...
                                // 26.2: Item#damageItem was a NeoForge extension; ItemStack#hurtAndBreak takes the raw amount.
                                toDamage.hurtAndBreak(1, citizen, EquipmentSlot.MAINHAND);
                            }
                            if (!ItemStackUtils.isEmpty(toDamage))
                            {
