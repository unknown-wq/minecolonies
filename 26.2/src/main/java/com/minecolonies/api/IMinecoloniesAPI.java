...
public interface IMinecoloniesAPI
{
    ...
    // TODO(port-26.2): DISABLED — NeoForge's NewRegistryEvent has no Fabric counterpart. Modded registries are
    // created eagerly with FabricRegistryBuilder (PORTING-BUNDLE "Custom (modded) registry"), so there is no
    // event to hand out; the hook is dropped from the API surface.
