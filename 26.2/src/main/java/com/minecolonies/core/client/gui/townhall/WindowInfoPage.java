...
public class WindowInfoPage extends AbstractWindowTownHall
{
    ...
    {
        // Called from onUpdate, i.e. 20x a second while the town hall is open. The stream/collect/addAll
        // chain that used to be here allocated a stream, a lambda capture and a whole intermediate list on
        // every tick; the plain loop is the same filter with none of that.
        workOrders.clear();
        for (final IWorkOrderView order : buildingView.getColony().getWorkOrders())
        {
            if (order.shouldShowIn(buildingView))
            {
