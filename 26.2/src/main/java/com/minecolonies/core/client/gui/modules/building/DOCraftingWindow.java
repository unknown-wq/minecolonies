...
public class DOCraftingWindow extends AbstractModuleWindow<DOCraftingModuleView>
{
    ...
    /**
     * PORT-26.2: {@code Level#getRecipeManager()} no longer exists and only the server side {@code RecipeAccess} is a
     * {@code RecipeManager}. Fabric's recipe sync API exposes the same lookup on both sides; Domum Ornamentum does
     * exactly this in {@code ArchitectsCutterContainer#updateAvailableRecipes}.
     *
     * @return every architect's cutter recipe matching the current input inventory.
     */
    private List<RecipeHolder<ArchitectsCutterRecipe>> cutterRecipes()
    {
        ...
        return level.recipeAccess()
                 ...
                 .<ArchitectsCutterRecipeInput, ArchitectsCutterRecipe>getAllMatches(ModRecipeTypes.ARCHITECTS_CUTTER.get(),
                   new ArchitectsCutterRecipeInput(inputInventory),
                   level)
    ...
    {
        final List<RecipeHolder<ArchitectsCutterRecipe>> list = cutterRecipes();
