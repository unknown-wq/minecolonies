...
public class FemaleMinerModel extends CitizenModel<AbstractEntityCitizen>
{
    ...
    public void setupAnim(@NotNull final CitizenRenderState state)
    {
        ...
        backpack.getChild("lid_r1").visible = !state.renderMetadata.contains(RENDER_META_STONE);
        backpack.getChild("torches").visible = state.renderMetadata.contains(RENDER_META_TORCH);
        backpack.getChild("shovel").visible = state.renderMetadata.contains(RENDER_META_SHOVEL);
        backpack.getChild("pick").visible = state.renderMetadata.contains(RENDER_META_PICKAXE);
