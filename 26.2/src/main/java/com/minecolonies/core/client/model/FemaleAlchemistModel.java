...
public class FemaleAlchemistModel extends CitizenModel<AbstractEntityCitizen>
{
    ...
    public void setupAnim(@NotNull final CitizenRenderState state)
    {
        ...
        head.getChild("PointHat").visible = state.pose != Pose.SLEEPING && displayHat(state);
        body.getChild("Potion1").visible = state.pose != Pose.SLEEPING;
        body.getChild("IngredientPouch").visible = state.pose != Pose.SLEEPING;
    }
}
