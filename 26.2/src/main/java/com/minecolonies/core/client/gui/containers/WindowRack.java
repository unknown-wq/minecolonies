...
import com.minecolonies.api.inventory.api.CombinedInvWrapper;
...
public class WindowRack extends AbstractContainerScreen<ContainerRack>
{
    ...
    public WindowRack(final ContainerRack container, final Inventory playerInventory, final Component iTextComponent)
    {
        ...
        // The row and column counts the 1.21.1 constructor derived here only ever fed imageHeight/imageWidth, which
        // are final in 26.2 and so are computed before the super call now; only the row count itself is still read,
        // by the background renderer.
        this.inventoryRows = jointChestInventory.getSlots() / INVENTORY_COLUMNS;
    }

    /**
     * @return the total slot count of the rack (and its neighbour, if any).
     */
    private static int slotCountOf(final ContainerRack container)
    {
        if (container.neighborRack != null)
        {
            return container.rack.getInventory().getSlots() + container.neighborRack.getInventory().getSlots();
        }
        return container.rack.getInventory().getSlots();
    }

    /**
     * @param container the container being opened.
     * @return the window height: the header plus one row per line of rack slots, capped at the scrollable maximum.
     */
    private static int imageHeightOf(final ContainerRack container)
    {
        final int rows = Math.min(slotCountOf(container) / INVENTORY_COLUMNS, INVENTORY_BAR_SIZE);
        return Y_OFFSET + rows * PLAYER_INVENTORY_OFFSET_EACH;
    }

    /**
     * @param container the container being opened.
     * @return the window width: the vanilla width, widened by the extra columns a rack too tall to draw in rows uses.
     */
    private static int imageWidthOf(final ContainerRack container)
    {
        final int size = slotCountOf(container);
        ...
        return columns > INVENTORY_COLUMNS
                 ? DEFAULT_IMAGE_WIDTH + (columns - INVENTORY_COLUMNS) * PLAYER_INVENTORY_OFFSET_EACH
                 : DEFAULT_IMAGE_WIDTH;
    }
    ...
    public void extractBackground(@NotNull final GuiGraphicsExtractor stack, final int mouseX, final int mouseY, final float partialTicks)
    {
        ...
        if (inventoryRows <= GOOD_SIZE)
        {
            ...
            stack.blit(RenderPipelines.GUI_TEXTURED, loc, this.leftPos, this.topPos + rowsHeight, 0,
