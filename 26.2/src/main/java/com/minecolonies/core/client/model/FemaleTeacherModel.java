...
public class FemaleTeacherModel extends CitizenModel<AbstractEntityCitizen>
{
    ...
    public void setupAnim(@NotNull final CitizenRenderState state)
    {
        ...
        head.getChild("feather_r1").visible = state.pose != Pose.SLEEPING;
        head.getChild("glasses").visible = state.pose != Pose.SLEEPING;
        head.getChild("HairExtension").visible = state.pose != Pose.SLEEPING;
        body.getChild("dress").visible = state.pose != Pose.SLEEPING;
        body.getChild("bag").visible = state.pose != Pose.SLEEPING && isWorking(state);
    }
}
