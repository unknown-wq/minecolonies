...
public class CitizenWindowUtils
{
    ...
    /**
     * Show a skill's icon on the job page.
     * <p>
     * The six icons start hidden: their source is filled in from the skill, and an unemployed citizen never reaches
     * this method at all, so leaving them visible would show the missing-texture placeholder that {@code Image} now
     * demands every declaration carry.
     *
     * @param windowCitizen the job window.
     * @param paneId        the id of the label the icon belongs to; the icon's own id is that plus the appendix.
     * @param skill         the skill whose icon to show.
     */
    private static void setSkillIcon(final JobWindowCitizen windowCitizen, final String paneId, final Skill skill)
    {
        final Image image = windowCitizen.findPaneOfTypeByID(paneId + IMAGE_APPENDIX, Image.class);
        image.setImage(Identifier.parse(BASE_IMG_SRC + skill.name().toLowerCase(Locale.US) + ".png"), false);
        image.show();
    }
    ...
    public static void updateJobPage(final ICitizenDataView citizen, final JobWindowCitizen windowCitizen, final IColonyView colony)
    {
        ...
        if (building instanceof AbstractBuildingView && building.getBuildingType() != ModBuildings.library.get() && citizen.getJobView() != null)
        {
            ...
            setSkillIcon(windowCitizen, PRIMARY_SKILL_LABEL, primary);
            ...
            if (primary.getComplimentary() != null && primary.getAdverse() != null)
            {
                ...
                setSkillIcon(windowCitizen, PRIMARY_SKILL_COM, primary.getComplimentary());
                ...
                windowCitizen.findPaneOfTypeByID(PRIMARY_SKILL_ADV, Text.class)
                  .setText(Component.translatableEscape(PARTIAL_SKILL_NAME + primary.getAdverse().name().toLowerCase(Locale.US)).append(" (-"
                                  + PRIMARY_DEPENDENCY_SHARE + "% XP)"));
                setSkillIcon(windowCitizen, PRIMARY_SKILL_ADV, primary.getAdverse());
            }
            ...
            setSkillIcon(windowCitizen, SECONDARY_SKILL_LABEL, secondary);
            ...
            if (secondary.getComplimentary() != null && secondary.getAdverse() != null)
            {
                ...
                setSkillIcon(windowCitizen, SECONDARY_SKILL_COM, secondary.getComplimentary());
                ...
                windowCitizen.findPaneOfTypeByID(SECONDARY_SKILL_ADV, Text.class)
                  .setText(Component.translatableEscape(PARTIAL_SKILL_NAME + secondary.getAdverse().name().toLowerCase(Locale.US)).append(" (-"
                                  + SECONDARY_DEPENDENCY_SHARE + "% XP)"));
                setSkillIcon(windowCitizen, SECONDARY_SKILL_ADV, secondary.getAdverse());
            }
        }
        else
        {
