...
public class MineColonies implements ModInitializer
{
    ...
    public void onInitialize()
    {
        ...
        CommonLifecycleEvents.TAGS_LOADED.register((registries, client) -> {
            ...
            // was: the `compostables` NeoForge data map, generated by DefaultDataMapsProvider. Fabric has no
            // data maps; CompostableRegistry is the runtime equivalent. Registered here rather than above for
            // the same reason as the line before it -- the table is derived from each item's FOOD component,
            // which is not readable during onInitialize. Re-adding on /reload just overwrites equal values.
            DefaultDataMapsProvider.compostables().forEach(CompostableRegistry.INSTANCE::add);
        });
