...
public class RecruitmentItemsListener extends SimpleJsonResourceReloadListener<JsonElement>
{
     ...
     *  <p>
     *  Port note (26.2): this used to be a constant. It cannot be any more -- {@code new ItemStack(item)} now
     *  reads the item's components through its holder, and those are not bound while mod initialisers run, so
     *  building the map in a static initialiser dies with {@code NullPointerException: Components not bound
     *  yet} the moment this class is touched from the entry point. Built on first use instead, i.e. from
     *  {@link #apply}, which {@link com.minecolonies.core.datalistener.DeferredDataListeners} holds back until
     *  after {@code ReloadableServerResources#updateComponentsAndStaticRegistryTags()} has bound them.
     */
    private static Map<Integer, ItemStack> rarityToBoot = null;

    /**
     * @return the recruitLevel-to-boot-tier map, built on first use.
     */
    private static Map<Integer, ItemStack> getRarityToBootMap()
    {
        if (rarityToBoot == null)
        {
            rarityToBoot = Map.ofEntries(
    ...
    {
        super(DataListenerUtils.JSON_CODEC, DataListenerUtils.dir(COLONY_RECRUITMENT_ITEMS_DIR));
    }
        ...
        final @NotNull ProfilerFiller profiler)
    {
        ...
        for (final Map.Entry<Identifier, JsonElement> entry : jsonElementMap.entrySet())
        {
            ...
            recruitCosts.get(rarity).add(new RecruitCost(new ItemStack(item, count), recruitLevel, getRarityToBootMap().get(rarity)));
        }
