...
public class QuestJsonListener extends SimpleJsonResourceReloadListener<JsonElement>
{
    ...
    {
        super(DataListenerUtils.JSON_CODEC, DataListenerUtils.dir(COLONY_QUESTS_DIR));
    }
    ...
    protected void apply(final Map<Identifier, JsonElement> jsonElementMap, final @NotNull ResourceManager resourceManager, final @NotNull ProfilerFiller profiler)
    {
        ...
        apply(DataListenerUtils.registryLookup(), jsonElementMap);
    }
    ...
    public static IQuestTemplate loadDataFromJson(@NotNull final HolderLookup.Provider provider, final Identifier questId, final JsonObject jsonObject) throws Exception
    {
        ...
        for (final JsonElement triggerJson : jsonObject.get(QUEST_TRIGGERS).getAsJsonArray())
        {
            ...
            try
            {
                questTriggers.add(IMinecoloniesAPI.getInstance().getQuestTriggerRegistry().getValue(Identifier.parse(type)).produce(triggerObj));
            }
        ...
        for (final JsonElement objectivesJson : jsonObject.get(QUEST_OBJECTIVES).getAsJsonArray())
        {
            ...
            try
            {
                questObjectives.add(IMinecoloniesAPI.getInstance().getQuestObjectiveRegistry().getValue(Identifier.parse(type)).produce(provider, objectiveObj));
            }
            catch (final Exception ex)
            {
        ...
        for (final JsonElement objectivesJson : jsonObject.get(QUEST_REWARDS).getAsJsonArray())
        {
            ...
            try
            {
                questRewards.add(IMinecoloniesAPI.getInstance().getQuestRewardRegistry().getValue(Identifier.parse(type)).produce(provider, objectiveObj));
            }
            catch (final Exception ex)
            {
        ...
        for (final JsonElement effectJson : jsonObject.get(QUEST_EFFECTS).getAsJsonArray())
        {
            ...
            final Identifier effectID = getResourceLocation(effectJsonObj.get(ID).getAsString());
