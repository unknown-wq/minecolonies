...
public class CustomRecipe
{
      ...
      @NotNull final JsonObject templateJson)
    {
        ...
        for (final Holder<Item> item : BuiltInRegistries.ITEM.getTagOrEmpty(TagKey.create(Registries.ITEM, tagId)))
        {
        ...
        if(mustExist)
        {
            ...
            for (final ICraftingBuildingModule module : building.getModulesByType(ICraftingBuildingModule.class))
            {
                for (IToken<?> recipeToken : module.getRecipes())
                {
                    ...
                    // 26.2/Fabric: the same unguarded lookup that took a colony's slow tick down for five minutes in
                    // AbstractCraftingBuildingModule#checkForWorkerSpecificRecipes, reached from the same call. This one
                    // needs a "must-exist" custom recipe to be live, which the shipped datapack has none of, but a pack
                    // that adds one would hit it first. A token with no storage cannot be the precursor being looked
                    // for, so skipping it is the whole answer here; the module's own tick prunes it.
                    if (storage == null)
                    {
                        continue;
                    }
