...
public class Permissions implements IPermissions
{
    ...
    public void loadPermissions(@NotNull final CompoundTag compound)
    {
        ...
        for (int i = 0; i < ownerTagList.size(); ++i)
        {
            ...
            final GameProfile player = ServerLifecycleHooks.getCurrentServer().services().profileResolver().fetchById(id).orElse(null);
    ...
    public void restoreOwnerIfNull()
    {
        ...
        {
            final GameProfile player = com.ldtteam.common.util.ServerLifecycleHooks.getCurrentServer().services().profileResolver().fetchById(ownerUUID).orElse(null);
    ...
    {
        // The per-colony off switch, /mc colony protection <colony> off. This is the single funnel every server
        // side permission test in the mod goes through, which is exactly why the switch is read here and nowhere
        // else. EDIT_PERMISSIONS is excluded on purpose: see ColonyProtection.
        if (action != Action.EDIT_PERMISSIONS && ColonyProtection.isOff(colony))
        {
            return true;
        }
        ...
        else if (player.permissions().hasPermission(new Permission.HasCommandLevel(PermissionLevel.byId(IMinecoloniesAPI.getInstance().getConfig().getServer().permissionEventMinBypassPermLevel.get()))) && player.isCreative())
        {
    ...
    {
        return getRank(player.getGameProfile().id());
    }
    ...
    public boolean setPlayerRank(final UUID id, final Rank rank, final Level world)
    {
        ...
        else
        {
            ...
            final GameProfile gameprofile = world.getServer().services().profileResolver().fetchById(id).orElse(null);
    ...
    public boolean addPlayer(@NotNull final String player, final Rank rank, final Level world)
    {
        ...
        final GameProfile gameprofile = world.getServer().services().profileResolver().fetchByName(player).orElse(null);
    ...
    {
        return players.containsKey(player.getGameProfile().id());
    }
