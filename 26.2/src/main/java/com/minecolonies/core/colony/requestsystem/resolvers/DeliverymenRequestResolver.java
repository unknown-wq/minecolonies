...
public abstract class DeliverymenRequestResolver<R extends IRequestable> extends AbstractRequestResolver<R>
{
    ...
    public boolean hasCouriers(@NotNull final IRequestManager manager)
    {
        ...
        // 26.2/Fabric: getAssignedCitizen() copies the whole assignee list into a fresh ArrayList, so the
        // old `!....getAssignedCitizen().isEmpty()` allocated one list per call purely to test emptiness.
        // This sits on the request-resolution path -- canResolveRequest and attemptResolveRequest both
        // reach it for every request against every candidate resolver -- and the copy is courier-sized,
        // so it grew with the warehouse cap. hasAssignedCitizen() is the allocation-free equivalent.
        return wareHouse.getModule(BuildingModules.WAREHOUSE_COURIERS).hasAssignedCitizen();
    }
    ...
    public void resolveRequest(@NotNull final IRequestManager manager, @NotNull final IRequest<? extends R> request) throws RuntimeException
    {
        ...
        if (!wareHouse.getModule(BuildingModules.WAREHOUSE_COURIERS).hasAssignedCitizen())
        {
            return;
        }
