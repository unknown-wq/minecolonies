...
public class AnimalData implements IAnimalData
{
    ...
    /**
     * The position of the home building, as read from NBT, until it can be resolved to a building.
     * <p>
     * PORT-NOTE(26.2): the home used to be resolved in {@link #deserializeNBT} and nowhere else, which is a load
     * order bet: the animal manager can be read before the building manager, and then every animal comes back from
     * disk homeless. A loose cavalry horse hides this because {@code ReturnToStableGoal#validateHomeStable} looks
     * its stable up again on the next tick - but that goal stands down while the horse is being ridden, so a
     * cavalry horse saved with its rider on it stayed homeless for as long as the ride lasted, with
     * {@code CavalryStrollGoal} and the stall lookup silently doing nothing. Keeping the position and resolving it
     * on demand makes the load order irrelevant.
     */
    @Nullable
    ...
    public CompoundTag serializeNBT(@NotNull final HolderLookup.Provider provider)
    {
        ...
        // homePos rather than homeBuilding, so a home that has not been resolved yet (see the field) is not
        // thrown away by the first save after loading.
        BlockPosUtil.write(compoundNBT, NbtTagConstants.TAG_ANIMALHOME,
          homeBuilding != null ? homeBuilding.getID() : (homePos == null ? BlockPos.ZERO : homePos));
        ...
        compoundNBT.store(NbtTagConstants.TAG_UUID, UUIDUtil.CODEC, uuid != null ? uuid : UUID.randomUUID());

        if (owner != null)
        {
            compoundNBT.store(NbtTagConstants.TAG_OWNER, UUIDUtil.CODEC, owner);
        }
    ...
    public void deserializeNBT(@NotNull final HolderLookup.Provider provider, final CompoundTag nbtTagCompound)
    {
        ...
        maxHealth = nbtTagCompound.contains(NbtTagConstants.TAG_MAX_HEALTH) ? nbtTagCompound.getFloatOr(NbtTagConstants.TAG_MAX_HEALTH, 0) : 0.0f;
        setCombatCooldown(nbtTagCompound.contains(NbtTagConstants.TAG_COMBAT_COOLDOWN) ? nbtTagCompound.getFloatOr(NbtTagConstants.TAG_COMBAT_COOLDOWN, 0) : 0.0f);
        uuid = nbtTagCompound.contains(NbtTagConstants.TAG_UUID) ? nbtTagCompound.read(NbtTagConstants.TAG_UUID, UUIDUtil.CODEC).orElse(null) : UUID.randomUUID();
        owner = nbtTagCompound.contains(NbtTagConstants.TAG_OWNER) ? nbtTagCompound.read(NbtTagConstants.TAG_OWNER, UUIDUtil.CODEC).orElse(null) : null;
    }
    ...
    {
        if (homeBuilding == null && homePos != null && colony != null && colony.getServerBuildingManager() != null)
        {
            homeBuilding = colony.getServerBuildingManager().getBuilding(homePos);
        }
        return homeBuilding;
    ...
    public void setHomeBuilding(@Nullable final IBuilding building)
    {
        ...
        homePos = building == null ? null : building.getID();
        markDirty();
    }
