    ...
    public static class CraftingModule extends AbstractCraftingBuildingModule.Crafting
    {
        ...
        public List<IGenericRecipe> getAdditionalRecipesForDisplayPurposesOnly(@NotNull Level world)
        {
            ...
            final List<TagKey<Item>> dyes = List.of(
                    ConventionalItemTags.WHITE_DYES, ConventionalItemTags.ORANGE_DYES, ConventionalItemTags.MAGENTA_DYES, ConventionalItemTags.LIGHT_BLUE_DYES,
                    ConventionalItemTags.YELLOW_DYES, ConventionalItemTags.LIME_DYES, ConventionalItemTags.PINK_DYES, ConventionalItemTags.GRAY_DYES,
                    ConventionalItemTags.LIGHT_GRAY_DYES, ConventionalItemTags.CYAN_DYES, ConventionalItemTags.PURPLE_DYES, ConventionalItemTags.BLUE_DYES,
                    ConventionalItemTags.BROWN_DYES, ConventionalItemTags.GREEN_DYES, ConventionalItemTags.RED_DYES, ConventionalItemTags.BLACK_DYES);
            for (final ItemStack item : IColonyManager.getInstance().getCompatibilityManager().getListOfAllItems())
            {
                // PORT(26.2): ArmorItem no longer exists; dyeable armour is armour by tag plus a dyed-colour component.
                if (!item.has(DataComponents.DYED_COLOR)
                      || !(item.is(ItemTags.HEAD_ARMOR) || item.is(ItemTags.CHEST_ARMOR) || item.is(ItemTags.LEG_ARMOR) || item.is(ItemTags.FOOT_ARMOR)))
                {
                    continue;
                }
                ...
                {
                    final List<ItemStack> dyeItems = StreamSupport.stream(BuiltInRegistries.ITEM.getTagOrEmpty(dyeTag).spliterator(), false)
                            .map(ItemStack::new).toList();
                    if (dyeItems.isEmpty()) { continue; }

                    final DyeColor dye = dyeItems.get(0).get(DataComponents.DYE);
                    if (dye != null)
                    {
        ...
        public IRecipeStorage getFirstRecipe(Predicate<ItemStack> stackPredicate)
        {
            ...
            if(recipe == null && stackPredicate.test(new ItemStack(Items.WOOL.pick(DyeColor.WHITE))))
            {
        ...
        public boolean holdsRecipe(final IToken<?> token)
        {
            ...
            return recipe.getPrimaryOutput().getItem() == Items.WOOL.pick(DyeColor.WHITE);
        }
        ...
        private List<ItemStorage> getWoolItems()
        {
            if (woolItems == null)
            {
                woolItems = StreamSupport.stream(BuiltInRegistries.ITEM.getTagOrEmpty(ItemTags.WOOL).spliterator(), false)
                  .filter(item -> !item.value().equals(Items.WOOL.pick(DyeColor.WHITE)))
        ...
        private IToken<?> getTokenForWool(ItemStorage wool)
        {
            final IRecipeStorage tempRecipe = RecipeStorage.builder()
                    .withInputs(ImmutableList.of(wool, new ItemStorage(new ItemStack(Items.DYE.pick(DyeColor.WHITE)))))
                    .withPrimaryOutput(new ItemStack(Items.WOOL.pick(DyeColor.WHITE)))
                    .build();
