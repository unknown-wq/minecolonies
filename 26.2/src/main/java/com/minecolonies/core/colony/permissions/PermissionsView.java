...
public class PermissionsView implements IPermissions
{
    ...
    {
        if (player.permissions().hasPermission(new Permission.HasCommandLevel(PermissionLevel.byId(IMinecoloniesAPI.getInstance().getConfig().getServer().permissionEventMinBypassPermLevel.get()))))
        {
            return true;
        }
