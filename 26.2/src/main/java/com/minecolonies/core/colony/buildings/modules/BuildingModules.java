...
public class BuildingModules
{
    ...
    // courier hut before COURIER_WORK below was scaled. Nothing in the fisherman is single-worker by
    // construction: pond memory lives on JobFisherman (per citizen, saved per citizen in NBT), the shore
    // spot is chosen per job, and the fishing rod is requested per citizen by checkForToolOrWeapon, so N
    // fishermen produce N rod requests without any hut-level bookkeeping. getBuildingLevel() matches the
    // curve LivingBuildingModule and COURIER_WORK already use: five workers at a level-five hut.
    public static final BuildingEntry.ModuleProducer<WorkerBuildingModule,WorkerBuildingModuleView> FISHER_WORK         =
      new BuildingEntry.ModuleProducer<>("fisher_work",
        () -> new WorkerBuildingModule(ModJobs.fisherman.get(), Skill.Focus, Skill.Agility, false, ICommonBuilding::getBuildingLevel),
        () -> WorkerBuildingModuleView::new);
    ...
    // PORT-NOTE(26.2): upstream pins this at a constant 1, so the only way to add a courier is to build
    // another hut. The warehouse has always been willing to take more of them -- CourierAssignmentModule
    // caps at buildingLevel * 2, i.e. ten at level five -- so the hut, not the warehouse, was the limit a
    // player actually hit. Scaling by hut level opens that headroom without touching the request system:
    // couriers claim work by moving a token out of the shared WarehouseRequestQueueModule into their own
    // per-citizen data store (JobDeliveryman#getCurrentTask), which is already how N huts share one
    // warehouse today. getBuildingLevel() matches LivingBuildingModule's curve and stays under the
    // warehouse's 2*level, so an equally-levelled warehouse never silently becomes the new bottleneck.
    public static final BuildingEntry.ModuleProducer<DeliverymanAssignmentModule,WorkerBuildingModuleView> COURIER_WORK        =
      new BuildingEntry.ModuleProducer<>("courier_work", () -> new DeliverymanAssignmentModule(ModJobs.delivery.get(), Skill.Agility, Skill.Adaptability, false, ICommonBuilding::getBuildingLevel),
        () -> WorkerBuildingModuleView::new);
    ...
    // No view producer. The idle tracker is read on the server by /mc colony warehousestock and by nothing on the
    // client, so a view would only add a payload proportional to the number of distinct item types in the warehouse
    // to every building sync, for a screen that does not exist.
    public static final BuildingEntry.ModuleProducer<WarehouseIdleTrackerModule, IBuildingModuleView> WAREHOUSE_IDLE     =
      new BuildingEntry.ModuleProducer<>("warehouse_idle", WarehouseIdleTrackerModule::new, null);
