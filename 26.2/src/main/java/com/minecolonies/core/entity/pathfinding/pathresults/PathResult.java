...
public class PathResult<T extends AbstractPathJob>
{
    ...
    /**
     * The pool the calculation was submitted to, kept so a cancelled job can be taken back out of its queue.
     */
    private ExecutorService submittedTo = null;
    ...
    public void cancel()
    {
        if (pathCalculation != null)
        {
            ...
            // Future#cancel marks the task cancelled but leaves it sitting in the pool's queue, which is a bounded
            // ArrayBlockingQueue of 10000 -- a cancelled job goes on holding its slot until the single worker thread
            // reaches it and throws it away. Taking it out here is what actually frees the slot.
            //
            // remove() rather than purge(): ThreadPoolExecutor#submit enqueues the very RunnableFuture it returns, so
            // this is a targeted removal of one known task, where purge() would walk all ten thousand entries under
            // the queue lock on every cancellation. If the job has already been taken off the queue and started,
            // remove() simply finds nothing and the interrupt from cancel(true) is what stops it.
            if (submittedTo instanceof final ThreadPoolExecutor pool && pathCalculation instanceof final Runnable task)
            {
                pool.remove(task);
            }
