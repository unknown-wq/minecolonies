...
public class EntityAIWorkComposter extends AbstractEntityAIInteract<JobComposter, BuildingComposter>
{
    ...
    private IAIState getMaterials()
    {
        ...
        final List<ItemStorage> list = getCompostables();
        if (list.isEmpty())
        {
            complain();
            return getState();
        ...
        if (worksWithoutMaterials()
              && supplyMaterialWithoutRequest(list.get(0).getItemStack(), list.get(0).getItemStack().getMaxStackSize()) > 0)
        {
            // The composter files its own StackList a few lines down rather than going through the shared request
            // chokepoint, so it is gated here. The first entry of the list is the honest thing to hand over: either
            // the player's own choice, or - when they have made none and free mode answered for them, see
            // getCompostables - one of the items the game composts anyway.
            final int freeSlot = InventoryUtils.findFirstSlotInItemHandlerWith(worker.getInventoryCitizen(), stack -> list.contains(new ItemStorage(stack)));
            if (freeSlot >= 0)
            {
                worker.setItemInHand(InteractionHand.MAIN_HAND, worker.getInventoryCitizen().getStackInSlot(freeSlot));
                return START_WORKING;
            }
        }
    ...
    /**
     * The items this composter puts in its barrels.
     * <p>
     * Normally the hut's Compostables list, which the player fills in with checkboxes and which starts empty
     * ({@code BuildingModules#ITEMLIST_COMPOSTABLE} uses the one argument {@link ItemListModule} constructor), so a
     * freshly built composter with a worker and five registered barrels stands at its hut and does nothing at all:
     * measured at 100% of samples in {@code GET_MATERIALS} with 0% movement, the only sign being a chat complaint
     * once every twelve thousand AI ticks.
     * <p>
     * In free mode that question is the wrong one to ask, exactly as it is for the beekeeper's flower list
     * ({@code EntityAIWorkBeekeeper#getFlowers}): the point of the switch is that a worker is never left waiting to
     * be told or supplied. So free mode answers it with every item the game will compost - the same set the GUI
     * offers as checkboxes. Answering rather than persisting is deliberate: nothing is written to the hut, so a
     * player who ticked nothing on purpose still has that choice when the switch goes off again, and outside free
     * mode the empty list keeps meaning exactly what it did.
     *
     * @return the items to compost, empty only if the player has chosen none and free mode is off.
     */
    private List<ItemStorage> getCompostables()
    {
        final List<ItemStorage> chosen = building.getModuleMatching(ItemListModule.class, m -> m.getId().equals(COMPOSTABLE_LIST)).getList();
        if (!chosen.isEmpty() || !worksWithoutMaterials())
        {
            return chosen;
        }

        return List.copyOf(IColonyManager.getInstance().getCompatibilityManager().getCompostInputs());
    }
