...
public abstract class AbstractEntityAIInteract<J extends AbstractJob<?, J>, B extends AbstractBuilding> extends AbstractEntityAISkill<J, B>
{
      ...
      final Runnable blockBreakAction)
    {
        ...
        {
            // 26.2: Item#inventoryTick(stack, ServerLevel, owner, @Nullable EquipmentSlot).
            // The tool sits in the citizen inventory, not an equipment slot, hence null.
            if (world instanceof final ServerLevel serverLevel)
            {
                tool.getItem().inventoryTick(tool, serverLevel, worker, null);
            }
        }
    ...
    public void searchForItems(final AABB boundingBox)
    {
        items = world.getEntitiesOfClass(ItemEntity.class, boundingBox)
                  ...
                  .filter(item -> item != null && item.isAlive() &&
                                    // PORT-NOTE(26.2/Fabric): NeoForge's Entity#getPersistentData is gone, so the
                                    // "PreventRemoteMovement" opt-out honoured on NeoForge cannot be read here.
