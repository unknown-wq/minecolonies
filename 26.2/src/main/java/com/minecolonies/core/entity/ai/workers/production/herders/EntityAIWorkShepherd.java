...
public class EntityAIWorkShepherd extends AbstractEntityAIHerder<JobShepherd, BuildingShepherd>
{
    ...
    private IAIState shearSheep()
    {
        ...
        if (worker.getMainHandItem() != null)
        {
            ...
            int enchantmentLevel = EnchantmentHelper.getItemEnchantmentLevel(Utils.getRegistryValue(Enchantments.FORTUNE, world), worker.getMainHandItem());
            ...
            if (!this.world.isClientSide())
            {
                ...
                for (int j = 0; j < qty; ++j)
                {
                    items.add(new ItemStack(Items.WOOL.pick(sheep.getColor())));
                }
            }

            sheep.playSound(SoundEvents.SHEEP_SHEAR, 1.0F, 1.0F);
            new LocalizedParticleEffectMessage(new ItemStack(Items.WOOL.pick(sheep.getColor())), sheep.getOnPos().above()).sendToTrackingEntity(worker);
