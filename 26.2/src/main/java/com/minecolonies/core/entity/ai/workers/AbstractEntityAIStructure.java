...
// 26.2/Fabric: NeoForge's TriPredicate is gone; Structurize ships the same three-argument shape.
...
public abstract class AbstractEntityAIStructure<J extends AbstractJobStructure<?, J>, B extends AbstractBuildingStructureBuilder> extends AbstractEntityAIInteract<J, B>
{
    ...
    public static <J extends AbstractJobStructure<?, J>, B extends AbstractBuildingStructureBuilder> ItemCheckResult hasListOfResInInvOrRequest(
      ...
      final boolean force)
    {
        if (placer.buildsWithoutResources())
        {
            // Nothing is required, hence nothing may be requested and nothing can be missing.
            return SUCCESS;
        }
        ...
        // The predicate is applied to every occupied inventory slot, and it used to open a fresh Stream over
        // requestedMap.keySet() on each one. Hoisting the keys into a plain array and looping gives the identical
        // answer — same comparator, same order, same short-circuit — without a Stream per slot.
        final ItemStack[] requestedStacks = new ItemStack[requestedMap.size()];
        int requestedIdx = 0;
        for (final ItemStorage storage : requestedMap.keySet())
        {
            requestedStacks[requestedIdx++] = storage.getItemStack();
        }

        final List<ItemStack> foundStacks = InventoryUtils.filterItemHandler(placer.getWorker().getInventoryCitizen(),
          itemStack -> {
              for (final ItemStack requested : requestedStacks)
              {
                  if (ItemStackUtils.compareItemStacksIgnoreStackSize(requested, itemStack))
                  {
                      return true;
                  }
              }
        ...
        {
            // Same shape as above: this used to allocate a Stream over foundStacks for every requested entry.
            requestedMap.entrySet().removeIf(entry -> {
                final ItemStack wanted = entry.getKey().getItemStack();
                if (ItemStackUtils.isEmpty(wanted))
                {
                    return true;
                }
                for (final ItemStack target : foundStacks)
                {
                    if (ItemStackUtils.compareItemStacksIgnoreStackSize(target, wanted))
                    {
                        return true;
                    }
                }
    ...
    /**
     * Check whether the current build has to happen without requiring any build materials.
     *
     * @return true if the builder must build without requesting, waiting for or consuming materials.
     */
    public boolean buildsWithoutResources()
    {
        return building != null && building.buildsWithoutResources();
    }
