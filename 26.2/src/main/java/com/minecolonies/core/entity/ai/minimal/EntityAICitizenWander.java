...
public class EntityAICitizenWander implements IStateAI
{
    ...
    private IState wanderAtLeisureSite()
    {
        ...
        if (blockEntity instanceof IBlueprintDataProviderBE)
        {
            ...
            {
                // 26.2: IBlueprintDataProviderBE#getInWorldCorners returns Structurize's own Tuple (net.minecraft.util.Tuple
                // is gone and both projects vendored their own); EntityNavigationUtils speaks MineColonies' one.
                final com.ldtteam.structurize.api.Tuple<BlockPos, BlockPos> corners =
                  ((IBlueprintDataProviderBE) blockEntity).getInWorldCorners();
                EntityNavigationUtils.walkToRandomPosWithin(citizen, 10, DEFAULT_SPEED,
                  new Tuple<>(corners.getA(), corners.getB()), citizen.level().isRaining());
