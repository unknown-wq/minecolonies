...
public class ItemBannerRallyGuards extends AbstractItemMinecolonies
{
    ...
    // TODO(port-26.2): DISABLED -- NeoForge's IItemExtension#onDroppedByPlayer(ItemStack, Player) has no
    // counterpart in Fabric or vanilla 26.2 (0 hits for onDroppedByPlayer in /opt/mc-src). Dropping a rally
    // banner therefore no longer deactivates the rally automatically; the player has to shift-right-click it
    // off first. Restoring it needs a Fabric callback wired from the mod entry point (see report).
