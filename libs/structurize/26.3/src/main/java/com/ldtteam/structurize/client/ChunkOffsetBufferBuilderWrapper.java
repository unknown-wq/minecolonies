...
public class ChunkOffsetBufferBuilderWrapper implements VertexConsumer
{
    ...
    // new abstract member in 26.3: UV3 is the sheeted-decal / glint texture channel. The attribute only
    // exists in DefaultVertexFormat.ENTITY_GLINT_SPECIAL, so for every format that reaches this wrapper the
    // write is silently dropped inside BufferBuilder. Straight pass-through: it is a texture coordinate the
    // caller already computed, not a position, so the chunk offset must not be applied to it.
    @Override
