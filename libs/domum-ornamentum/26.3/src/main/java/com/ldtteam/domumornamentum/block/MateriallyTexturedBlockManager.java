...
public final class MateriallyTexturedBlockManager implements IMateriallyTexturedBlockManager
{
    ...
    /**
     * Lazily computed and then immutable. Both fields are published together, and only once the list is
     * complete: the Architect's Cutter asks for these from the render thread and from the server thread at
     * the same time on a single player world, and a half-built list would have been observable — see
     * {@link IMateriallyTexturedBlock#fillDOItemCategory} for the same defect where it was actually reported.
     */
    private volatile List<IMateriallyTexturedBlock> blocks = Lists.newArrayList();
    private volatile int maxTexturableComponentCount = -1;

    @Override
    public int getMaxTexturableComponentCount() {
        final int known = this.maxTexturableComponentCount;
        if (known >= 0)
            return known;
        ...
        synchronized (this) {
            ...
            final List<IMateriallyTexturedBlock> found = StreamSupport.stream(BuiltInRegistries.BLOCK.spliterator(), false)
              .filter(IMateriallyTexturedBlock.class::isInstance)
              .map(IMateriallyTexturedBlock.class::cast)
              .collect(Collectors.toList());

            final int count = found.stream()
              .map(IMateriallyTexturedBlock::getComponents)
              .mapToInt(Collection::size)
              .max()
              .orElse(0);

            this.blocks = List.copyOf(found);
            this.maxTexturableComponentCount = count;
            return count;
        }
    }
