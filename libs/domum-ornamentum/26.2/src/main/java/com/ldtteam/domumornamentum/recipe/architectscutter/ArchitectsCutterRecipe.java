...
public class ArchitectsCutterRecipe implements Recipe<ArchitectsCutterRecipeInput>
{
    ...
    /**
     * 26.2: {@code Recipe#getResultItem(HolderLookup.Provider)} was removed from the interface. Kept
     * as a plain helper because the architect's cutter menu needs the "what would this recipe make"
     * preview without any inputs.
     */
    ...
    /**
     * The architect's cutter is not a recipe-book / auto-placeable recipe: the inputs are not
     * ingredients but skins picked in a custom GUI.
     * <p>
     * This MUST NOT return null. {@code RecipeManager#finalizeRecipeLoading} calls
     * {@code recipe.placementInfo().isImpossibleToPlace()} on every loaded recipe
     * (/opt/mc-src/net/minecraft/world/item/crafting/RecipeManager.java:98) and a null here NPEs the
     * whole datapack recipe load. {@link #isSpecial()} keeps that same call from logging
     * "can't be placed ... will be ignored" once per cutter recipe.
     */
    @Override
