...
public class ArchitectsCutterContainer extends AbstractContainerMenu
{
    ...
    public ArchitectsCutterContainer(int windowIdIn, Inventory playerInventoryIn, final ContainerLevelAccess worldPosCallableIn) {
            ...
            this.inputInventorySlots.add(
              this.addSlot(new Slot(this.inputInventory, i, x, y) {
                  ...
                  public boolean mayPlace(@NotNull final ItemStack stack)
                  {
                      ...
                      {
                          return MateriallyTexturedBlockManager.getInstance().doesItemStackContainsMaterialForSlot(this.getContainerSlot(), stack);
                      }
                      else
                      {
                          return MateriallyTexturedBlockManager.getInstance().doesItemStackContainsMaterialForSlot(this.getContainerSlot(), stack, currentVariant);
                      }
                  }
              })
            );
        ...
        this.outputInventorySlot = this.addSlot(new Slot(this.inventory, 1, CUTTER_OUTPUT_X, CUTTER_OUTPUT_Y) {
            ...
            public void onTake(@NotNull Player thePlayer, @NotNull ItemStack stack) {
                // 26.2: ItemStack#onCraftedBy lost its Level parameter (ItemStack.java:721).
    ...
    private void updateAvailableRecipes(Container inventoryIn, List<ItemStack> stacks) {
        ...
        if (!stacks.stream().allMatch(ItemStack::isEmpty)) {
            // 26.2: Level#getRecipeManager() is gone. Level#recipeAccess() is a RecipeAccess, and only the
            // server's is a RecipeManager. Fabric's recipe-sync API exposes the same lookup on both sides
            // (fabric-recipe-api-v1: FabricRecipeAccess#getSynchronizedRecipes ->
            // SynchronizedRecipes#getAllMatches). This requires ModRecipeSerializers to call
            // RecipeSynchronization.synchronizeRecipeSerializer(...) on the cutter serializer.
            this.recipes = this.world.recipeAccess()
                .getSynchronizedRecipes()
                .<ArchitectsCutterRecipeInput, ArchitectsCutterRecipe>getAllMatches(ModRecipeTypes.ARCHITECTS_CUTTER.get(),
                    new ArchitectsCutterRecipeInput(inventoryIn),
                    this.world)
                .collect(Collectors.toCollection(Lists::newArrayList));
            this.recipes.sort(Comparator.<RecipeHolder<ArchitectsCutterRecipe>, Identifier>comparing(h -> h.value().getBlockName())
            ...
            for (final RecipeHolder<ArchitectsCutterRecipe> recipeHolder : recipes)
            {
                ...
                if (resultItem.getItem() == currentVariant.getItem())
                {
                    ...
                    if ((resultBlockState.isEmpty() && currentBlockState.isEmpty()) || resultBlockState.equals(currentBlockState))
                    {
                        ...
                        // 26.2: Recipe#assemble(T input) lost the HolderLookup.Provider argument.
