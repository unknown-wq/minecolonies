...
public class DynamicTimberFrameBlockEntity extends AbstractMateriallyTexturedBlockEntity
{
    ...
    @Override
    public void saveToItem(@NotNull final ItemStack stack, final HolderLookup.Provider provider)
    {
        // 26.2: BlockEntity#saveToItem is gone and BlockItem#setBlockEntityData takes a TagValueOutput
        // instead of a CompoundTag — mirrors
        // /opt/mc-src/net/minecraft/server/network/ServerGamePacketListenerImpl.java:708-716.
    ...
    protected void saveAdditional(final ValueOutput output)
    {
        ...
        output.store(BLOCK_ENTITY_TEXTURE_DATA, MaterialTextureData.CODEC, originalTextureData);
        output.putString(PRIMARY_BLOCK, BuiltInRegistries.BLOCK.getKey(centerBlock).toString());
        output.putString(SECONDARY_BLOCK, BuiltInRegistries.BLOCK.getKey(frameBlock).toString());

        // Kept byte-identical to the old ListTag of {offset:int, bool:boolean} compounds: a
        // ValueOutput children list serialises to exactly that.
        final ValueOutput.ValueOutputList offsetList = output.childrenList(OFFSETS);
        for (final Object2BooleanMap.Entry<DynamicTimberFrameBlock.Offset> mapEntry : offsets.object2BooleanEntrySet())
        {
            final ValueOutput localCompound = offsetList.addChild();
    ...
    protected void loadAdditional(final ValueInput input)
    {
        ...
        final Block primaryBlock = BuiltInRegistries.BLOCK.getValue(Identifier.parse(input.getStringOr(PRIMARY_BLOCK, "minecraft:air")));
        if (primaryBlock != null && primaryBlock != Blocks.AIR)
        {
            this.centerBlock = primaryBlock;
        }

        final Block secondaryBlock = BuiltInRegistries.BLOCK.getValue(Identifier.parse(input.getStringOr(SECONDARY_BLOCK, "minecraft:air")));
        if (secondaryBlock != null && secondaryBlock != Blocks.AIR)
        {
            this.frameBlock = secondaryBlock;
        }

        offsets.clear();
        for (final ValueInput localCompound : input.childrenListOrEmpty(OFFSETS))
        {
            final int ordinal = localCompound.getIntOr("offset", -1);
            if (ordinal >= 0 && ordinal < DynamicTimberFrameBlock.Offset.values().length)
            {
                offsets.put(DynamicTimberFrameBlock.Offset.values()[ordinal], localCompound.getBooleanOr("bool", false));
            }
        }
    ...
    /**
     * Fabric's replacement for NeoForge's {@code getModelData()} — see
     * {@link MateriallyTexturedBlockEntity#getRenderData()}.
     */
    @Override
