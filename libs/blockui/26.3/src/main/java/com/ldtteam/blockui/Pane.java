...
public class Pane extends UiRenderMacros
{
    ...
    {
        // TODO(port-26.3): DEGRADED — the `key` handed to the deprecated onKeyTyped bridge changed
        // meaning with no signature change: in 26.2 KeyEvent#key() was a GLFW keysym, in 26.3 it is
        // an SDL scancode (API-CHECKLIST-26.3.md D1). Left as key() on purpose: neither SDL family
        // reproduces a GLFW keysym exactly (scancode = physical position, keycode = layout-mapped
        // symbol), the method is @Deprecated(forRemoval) and nothing in BlockUI or the three ldtteam
        // consumers overrides it, so there is no live behaviour to preserve. Anyone reviving this
        // hook must decide between key() and shortcutKey() explicitly.
