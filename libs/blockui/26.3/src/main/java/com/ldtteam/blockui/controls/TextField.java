...
public class TextField extends Pane
{
    ...
    {
        // 26.3: text editing must key off event.shortcutKey() (an SDL *keycode*, layout aware),
        // not event.key() (an SDL *scancode*, physical position) — API-CHECKLIST-26.3.md D1.
        // Vanilla EditBox.keyPressed does exactly this
        // (/opt/mc-src-26.3/net/minecraft/client/gui/components/EditBox.java:273-336), hence the
        // KEYCODE_* family below. InputConstants has no KEYCODE_ESCAPE, so that one comes from SDL.
        switch (event.shortcutKey())
        {
            ...
            case InputConstants.KEYCODE_TAB:
                return handleTab();
    ...
    {
        final int direction = (event.shortcutKey() == InputConstants.KEYCODE_LEFT) ? -1 : 1;
    ...
    {
        final int position = (event.shortcutKey() == InputConstants.KEYCODE_HOME) ? 0 : text.length();
    ...
    {
        final int direction = (event.shortcutKey() == InputConstants.KEYCODE_BACKSPACE) ? -1 : 1;
