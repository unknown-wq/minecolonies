...
public interface ICitizenData extends ICivilianData, IQuestGiver, IQuestParticipant
{
    ...
    /**
     * How many colony days of life this citizen has used up.
     * <p>
     * Not a simple day count: the generational mechanic adds more or less than one per colony day depending on how
     * well the citizen is living, so a miserable homeless citizen burns through a life faster than a happy housed one.
     * {@link #AGE_UNKNOWN} means the citizen has never been aged -- either it predates the mechanic in this save, or
     * the mechanic has been off since it was born.
     *
     * @return the used life, in colony days, or {@link #AGE_UNKNOWN}.
     */
    double getAgeDays();

    /**
     * Set the used life, in colony days.
     *
     * @param ageDays the new value.
     */
    void setAgeDays(double ageDays);

    /**
     * The citizen's personal deviation from the configured lifespan, as a multiplier.
     * <p>
     * Rolled once and stored, so that a cohort born on the same day does not die on the same day. Kept as a
     * multiplier rather than as an absolute expectancy so that changing {@code generationslifespandays} takes effect
     * on citizens who are already alive.
     *
     * @return the multiplier, or {@link #LIFE_JITTER_UNSET} if it has not been rolled yet.
     */
    float getLifeJitter();

    /**
     * Set the personal lifespan multiplier.
     *
     * @param lifeJitter the new value.
     */
    void setLifeJitter(float lifeJitter);

    /**
     * Sentinel for {@link #getAgeDays()}: this citizen has never been aged.
     */
    double AGE_UNKNOWN = -1.0;

    /**
     * Sentinel for {@link #getLifeJitter()}: this citizen's lifespan deviation has not been rolled yet.
     */
    float LIFE_JITTER_UNSET = 0.0f;

    /**
     * Sentinel for {@link #getParentIds()}: no citizen of this colony is known to be that parent. Zero, because that is
     * already what {@code partner} uses for "nobody" and citizen ids start at one.
     */
    int NO_PARENT = 0;
    ...
    /**
     * The ids of the parents, {@link #NO_PARENT} where the parent is not a citizen this colony can name.
     * <p>
     * 26.2/Fabric: parents were stored as names alone while every other family link -- siblings, children, partner --
     * is an id, so nothing could locate a parent and two citizens sharing a name were indistinguishable. The names are
     * kept alongside for display and for saves written before this, and they remain the answer when the id has gone
     * stale: an id is dropped as soon as the citizen it names is gone or has been reused by somebody else, but "her
     * mother was called Marie" stays true after her mother is dead.
     *
     * @return the two ids.
     */
    Tuple<Integer, Integer> getParentIds();
    ...
    /**
     * Set the parents of the citizen, by id as well as by name.
     *
     * @param firstParentId  the first parent's citizen id, or {@link #NO_PARENT}.
     * @param secondParentId the second parent's citizen id, or {@link #NO_PARENT}.
     * @param firstParent    the first parent's name.
     * @param secondParent   the second parent's name.
     */
    void setParents(final int firstParentId, final int secondParentId, final String firstParent, final String secondParent);
    ...
    /**
     * Where the citizen goes at the end of the day.
     * <p>
     * <b>This is not "the citizen's home".</b> For a citizen with no residence it falls back to the nearest tavern, and
     * then to the town hall and the colony centre, so that the sleep AI and the unstick teleport always have somewhere
     * to send it. Ask {@link #getHomeBuilding()} instead when the question is whether the citizen has a home at all --
     * anything that reports a home to the player has to, or it tells them a homeless citizen lives in the tavern and
     * they have no way to tell that apart from actually living there.
     *
     * @return the pos to go home to, never null for a citizen in a colony.
     */
    @Nullable
