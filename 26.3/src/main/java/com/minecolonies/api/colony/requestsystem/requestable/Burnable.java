...
public class Burnable implements IDeliverable
{
    ...
    public static CompoundTag serialize(@NotNull final HolderLookup.Provider provider, final IFactoryController controller, final Burnable burnable)
    {
        ...
        {
            compound.put(NBT_RESULT, Utils.serializeCodecMess(ItemStack.OPTIONAL_CODEC, provider, burnable.result));
        }
    ...
    {
        // 26.2: FurnaceBlockEntity#isFuel is gone; fuel is a per-level FuelValues lookup
        // (/opt/mc-src/net/minecraft/world/level/Level.java:1107). The mod keeps its own fuel list, so the
        // compatibility manager's answer is used instead. The ignoreDamage/ignoreNBT flags are mandatory:
        // FuelValues#isFuel is `values.containsKey(stack.getItem())` (FuelValues.java:26-28), i.e. a
        // by-item test, while the plain ItemStorage(stack) ctor compares damage and components too - which
        // rejected every damaged wooden tool, bow and fishing rod that vanilla still burns.
        // ItemStorage#hashCode is Objects.hash(stack.getItem()), so the set lookup stays O(1).
        return com.minecolonies.api.colony.IColonyManager.getInstance().getCompatibilityManager().getFuel().contains(new com.minecolonies.api.crafting.ItemStorage(stack, true, true));
    }
