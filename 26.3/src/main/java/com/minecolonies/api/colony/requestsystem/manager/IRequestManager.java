...
public interface IRequestManager extends INBTSerializable<CompoundTag>, ITickable
{
    ...
    /**
     * Whether {@link #log(String)} will actually do anything. This is off by default (config
     * {@code rsEnableDebugLogging}), so guard every {@code log(...)} call whose argument has to be built -
     * string concatenation happens before the call and is not free, and the request system logs on every
     * request creation, assignment and reassignment.
     *
     * @return true if request system debug logging is enabled.
     */
