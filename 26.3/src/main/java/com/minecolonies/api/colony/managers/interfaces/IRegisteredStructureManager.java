...
public interface IRegisteredStructureManager extends ICommonRegisteredStructureManager<IBuilding, ITownHall>
{
    ...
    /**
     * Get a house with a spare bed, the one nearest the colony centre.
     *
    ...
    /**
     * Get the house a citizen should live in: the one with a spare bed nearest to where the citizen works.
     * <p>
     * This is the colony's single answer to "which house is nearest", shared by the automatic housing in
     * {@link com.minecolonies.core.colony.buildings.modules.LivingBuildingModule} and by the {@code /mc colony rehouse}
     * command, so the two can never disagree. Upstream returned the first house with a free bed in map iteration
     * order, with no distance term at all, which in a colony with an enclave hands a worker a bed a kilometre from
     * its work building.
     * <p>
     * The candidate list is every unlocked house with a free bed, plus the citizen's current house whether or not it
     * is full or locked. Ties are broken by position so the answer does not depend on map iteration order.
     *
     * @param citizen the citizen to house, or null to ask from the colony centre for nobody in particular.
     * @return the house or null.
     */
    IBuilding getHouseWithSpareBed(@Nullable ICitizenData citizen);

    /**
     * The position housing distance is measured from for a citizen: its work building, or the colony centre when it
     * has no job.
     *
     * @param citizen the citizen, or null for the colony centre.
     * @return the anchor position.
     */
    BlockPos getHousingAnchor(@Nullable ICitizenData citizen);
