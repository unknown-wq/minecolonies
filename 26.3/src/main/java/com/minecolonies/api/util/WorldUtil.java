...
public class WorldUtil
{
    ...
    public static boolean isChunkLoaded(final LevelAccessor world, final int x, final int z)
    {
        ...
        {
            final ChunkHolder holder = ((ServerChunkCache) world.getChunkSource()).chunkMap.visibleChunkMap.get(ChunkPos.pack(x, z));
            if (holder != null)
            {
    ...
    {
        // 26.2: the 1.21.1 getDayTime() read the level data, which for a non-overworld ServerLevel was a
        // DerivedLevelData delegating to the overworld. getDefaultClockTime() is NOT that: it reads
        // dimensionType().defaultClock() (Level.java:891) and getClockTimeTicks returns 0 for Optional.empty(),
        // which is exactly what the nether has (DimensionTypes.java:99) - so a nether colony would be in
        // permanent daytime. getOverworldClockTime() (Level.java:887) is the equivalent, and the clocks are
        // server-wide (ServerLevel#clockManager -> server.clockManager()).
    ...
    {
        // 26.2: see isDayTime above for why this is getOverworldClockTime and not getDefaultClockTime.
    ...
    {
        final RegistryAccess dynRegistries = world.registryAccess();
        // 26.2: RegistryAccess#registry -> #lookup (Optional<Registry<E>>); DimensionType#effectsLocation is gone,
        // so the old client-side fallback through the effects id has no replacement and is dropped.
        final Identifier loc = dynRegistries.lookup(Registries.DIMENSION_TYPE)
                                 .map(registry -> registry.getKey(world.dimensionType()))
                                 .orElse(null);
        if (loc == null)
        {
            return false;
        }
        return ResourceKey.create(Registries.DIMENSION_TYPE, loc).equals(type);
    }
    ...
    {
        return !(world instanceof final ServerLevel serverLevel && serverLevel.getGameRules().get(GameRules.SPAWN_MOBS)) || world.getDifficulty().equals(Difficulty.PEACEFUL);
    }
    ...
    public static <T extends LivingEntity> T getNearestEntity(List<? extends T> entityList, @Nullable Mob livingEntity, int x, int y, int z, double lookDistance)
    {
        ...
        for (T entity : entityList)
        {
            ...
            // 26.3: LivingEntity#getVisibilityPercent takes the ServerLevel now (it reads item components off
            // the entity's equipment). This helper is only ever called server-side; off the server there is no
            // sensible answer, and "fully visible" is the value the method itself starts from.
            double invisPct = entity.level() instanceof final ServerLevel serverLevel
                                ? entity.getVisibilityPercent(serverLevel, livingEntity)
                                : 1.0D;
