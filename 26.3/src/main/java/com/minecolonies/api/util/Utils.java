...
public final class Utils
{
    ...
    {
        // 26.2: Level#holderOrThrow is gone; go through the registry access.
        return level.registryAccess().lookupOrThrow((net.minecraft.resources.ResourceKey) resourceKey.registryKey()).getOrThrow(resourceKey);
    }
    ...
    /**
     * Append the written contents of a scratch buffer to an outgoing packet buffer, in the exact wire format
     * {@link net.minecraft.network.FriendlyByteBuf#writeByteArray(byte[])} produces (a VarInt length followed by
     * that many bytes), so the reading side keeps using {@code readByteArray()} unchanged.
     * <p>
     * This exists because {@code writeByteArray(scratch.array())} sends the whole <em>backing array</em>. Those
     * scratch buffers are {@code Unpooled.buffer()}, whose capacity is a power of two of at least 256 that grows by
     * doubling, so everything between {@code writerIndex()} and {@code array().length} is uninitialised padding that
     * goes out over the network. Measured on netty 4.2.15 (the build's own dependency): the amplification is 4/3
     * within every capacity octave, up to 2x just past a power of two.
     *
     * @param out    the outgoing packet buffer.
     * @param source the scratch buffer whose written contents should be sent.
     */
    public static void writeBufferContents(@NotNull final FriendlyByteBuf out, @NotNull final ByteBuf source)
    {
