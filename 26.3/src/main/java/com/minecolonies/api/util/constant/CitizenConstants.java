...
public final class CitizenConstants
{
    ...
    /**
     * The limit of citizens if the last increase research is done. This is the top rung of the citizen cap research ladder, and it is datagen input: it is what ends up in
     * data/minecolonies/researches/effects/citizencapaddition.json, so it cannot be changed without regenerating that file.
     */
    public static final int CITIZEN_LIMIT_MAX = 1000;

    /**
     * The highest value {@code maxcitizenpercolony} may be configured to. Only a ceiling on the config option: a colony is still capped by its beds and by the citizen cap
     * research, so raising the config above {@link #CITIZEN_LIMIT_MAX} only has an effect where the research cap does not apply.
     */
    public static final int CITIZEN_LIMIT_CONFIG_MAX = 1000;
