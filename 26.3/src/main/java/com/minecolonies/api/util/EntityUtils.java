...
public final class EntityUtils
{
    ...
    public static Player getPlayerOfFakePlayer(@NotNull final Player player, @NotNull final Level world)
    {
        ...
        // (net.fabricmc.fabric.api.entity.FakePlayer, in fabric-events-interaction-v0), so the upstream type test
        // is restored. The class-name heuristic that stood here was wider than upstream: every foreign ServerPlayer
        // subclass from any other mod passed it, and such a player then had its identity silently swapped for
        // whatever entity shares its UUID — which is exactly the object colony permissions are resolved against.
    ...
    /**
     * Whether the entity really made it into the level it points at.
     * <p>
     * PORT-NOTE(26.2/Fabric): upstream asks NeoForge's Entity#isAddedToLevel(), a flag NeoForge raises from the level
     * callback that starts tracking an entity and clears when tracking ends. Vanilla has no such flag, and
     * !entity.isRemoved() is not a replacement for it: ServerLevel#addFreshEntity (ServerLevel.java:992-999) returns
     * false without ever touching the removal state when the entity manager refuses the entity - a UUID that is
     * already known is the usual reason (PersistentEntitySectionManager#addEntityUuid). Looking the entity up by its
     * network id asks the storage that NeoForge's flag mirrors, and it is the same test that
     * CitizenColonyHandler#registerWithColony has always used, upstream included.
     *
     * @param entity the entity to check.
     * @return true if the level knows this exact entity.
     */
    public static boolean isAddedToLevel(@NotNull final Entity entity)
    {
    ...
    {
        return target != null && !target.onGround() && target.fallDistance <= 0.1f && target.level().isEmptyBlock(target.blockPosition().below(2));
    }
