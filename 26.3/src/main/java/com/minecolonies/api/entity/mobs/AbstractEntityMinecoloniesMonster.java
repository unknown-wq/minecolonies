...
public abstract class AbstractEntityMinecoloniesMonster extends AbstractFastMinecoloniesEntity implements IThreatTableEntity, Enemy
{
    ...
    {
        this.getAttribute(RaiderMobUtils.MOB_ATTACK_DAMAGE_HOLDER).setBaseValue(baseDamage);
    ...
    {
        // 26.2: Entity#hurt is final and only dispatches; the override point is hurtServer
        // (/opt/mc-src/net/minecraft/world/entity/Entity.java:1918-1931).
    ...
    public static AttributeSupplier.Builder getDefaultAttributes()
    {
        return LivingEntity.createLivingAttributes()
                .add(RaiderMobUtils.MOB_ATTACK_DAMAGE_HOLDER)
    ...
    {
        final long packed = input.getLongOr(TAG_SPAWN_POS, Long.MIN_VALUE);
        if (packed != Long.MIN_VALUE)
        {
            this.spawnPos = BlockPos.of(packed);
        }
     ...
     * @return
     */
    // PORT-NOTE(26.2): Mob#shouldDespawnInPeaceful() is indeed gone, but the behaviour is not — it moved into the
    // entity type. Mob#checkDespawn (/opt/mc-src/net/minecraft/world/entity/Mob.java:685-688) discards the mob when
    // the difficulty is PEACEFUL and !getType().isAllowedInPeaceful(), and that flag is set with
    // EntityType.Builder#notInPeaceful() (EntityType.java:593-596). It defaults to true (EntityType.java:479) and
    // MobCategory.MONSTER does not change it, so the flag has to be set explicitly.
    // It now is, on all 36 monster types in EntityInitializer — every type whose entity class descends from this
    // one, camp mobs included. Nothing is lost; do not re-open this.
}
