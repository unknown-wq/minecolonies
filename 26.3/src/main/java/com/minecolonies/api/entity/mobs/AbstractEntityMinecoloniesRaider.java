...
public abstract class AbstractEntityMinecoloniesRaider extends AbstractEntityMinecoloniesMonster implements IThreatTableEntity, Enemy
{
    ...
    public AbstractEntityMinecoloniesRaider(final EntityType<? extends AbstractEntityMinecoloniesRaider> type, final Level world, final int textureCount)
    {
        ...
        // 26.3: Entity.setInvulnerable(boolean) -> setPermanentlyInvulnerable(boolean) (same "Invulnerable" NBT flag).
    ...
    {
        // 26.2: entity NBT is codec-based, CompoundTag -> ValueOutput/ValueInput (Entity.java:2208/2210).
        output.putLong(TAG_TIME, worldTimeAtSpawn);
    ...
    {
        worldTimeAtSpawn = input.getLongOr(TAG_TIME, 0L);
        eventID = input.getIntOr(TAG_EVENT_ID, 0);
        {
            final int colonyId = input.getIntOr(TAG_COLONY_ID, 0);
            if (colonyId != 0)
            {
    ...
    {
        // 26.2: Entity#hurt is final and only dispatches; the override point is hurtServer (Entity.java:1918-1931).
        // PORT-NOTE(26.2/Fabric): Fabric API *does* ship a FakePlayer
        // (net.fabricmc.fabric.api.entity.FakePlayer, in fabric-events-interaction-v0), so the upstream
        // fake-player branch is restored. Upstream deliberately counts fake-player damage as environmental and
        // sends it through tempEnvDamageImmunity / envDmgCooldown / getMinRemainingHealthForEnvironmentalDamage.
        // Without it a raider can be ground down to death by automation driven off a fake player, which the
        // remaining-health floor exists precisely to prevent.
        ...
        else if (!level().isClientSide())
        {
            ...
            if (source instanceof final Player player)
            {
                ...
                {
                    source.hurtServer(serverLevel, level().damageSources().thorns(this), damage * 0.5f);
                }

                final float raiderDamageEnchantLevel =
                    EnchantmentHelper.getItemEnchantmentLevel(
                        player.level().registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(ModEnchants.raiderDamage),
                ...
                return super.hurtServer(serverLevel, damageSource, totalWithScaled);
            }
        }
