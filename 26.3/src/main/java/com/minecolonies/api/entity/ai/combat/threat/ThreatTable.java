...
public class ThreatTable<T extends LivingEntity & IThreatTableEntity>
{
    ...
    public void addThreat(final LivingEntity attacker, final int additionalThreat)
    {
        ...
        // (net.fabricmc.fabric.api.entity.FakePlayer, in fabric-events-interaction-v0), so the upstream filter
        // is restored verbatim. Without it the mod's own fake players — the ones that break blocks
        // (AbstractEntityAIBasic#getFakePlayer) and milk cows (EntityAIWorkCowboy) — climb to the top of the
        // threat table, and TargetAI then discards them again in isEntityValidTarget, so the guard resets its
        // target instead of attacking the real enemy.
