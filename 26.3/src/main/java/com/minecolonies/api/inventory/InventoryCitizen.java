...
public class InventoryCitizen implements IItemHandlerModifiable, Nameable
{
     ...
     * <p>
     * {@link #freeSlots} is a cache, and it is only maintained by the methods of this class that own a slot:
     * {@link #insertItem}, {@link #extractItem}, {@link #setStackInSlot} and the two shrink helpers. It cannot see a
     * slot emptied through the {@link ItemStack} itself, and {@link #getStackInSlot} hands that live stack to anyone
     * who asks. Two callers in the tree do exactly that and are not doing anything wrong by it: a citizen eating its
     * last piece of food goes through {@code ItemStack#finishUsingItem}, which shrinks the stack in place, and
     * {@code InventoryUtils#shrinkItemCountInItemHandler} calls {@code shrink(1)} on the stack it finds. Either one
     * leaves an empty slot behind with the counter still counting it as occupied.
     * <p>
     * The drift only ever runs one way -- the counter under-reports free slots, never over-reports them -- so the
     * symptom is a citizen that believes it is full while its inventory grid plainly is not, refusing work and
     * walking off to dump nothing. {@link #write} recomputes the counter from scratch, so it heals itself on the next
     * colony save or citizen sync, which is why it comes and goes rather than sticking.
     * <p>
     * Rather than chase every caller that may hold a live stack, the counter is treated as what it is -- an
     * optimisation -- and verified before it is allowed to deny a citizen a slot. The fast path is unchanged: a
     * counter that says there is room is believed without a scan, and only the "no room" answer costs a walk of the
     * (at most 54 slot) list.
     *
    ...
    /**
     * Recount {@link #freeSlots} from the slots themselves, discarding the cached value.
     */
    private void recountFreeSlots()
    {
        int free = 0;
        for (final ItemStack stack : mainInventory)
        {
    ...
    public <T extends LivingEntity> boolean damageInventoryItem(final int slot, int amount, @Nullable T entityIn, @Nullable Consumer<Item> onBroken)
    {
        ...
        if (!ItemStackUtils.isEmpty(stack))
        {
            ...
            // 26.2: Item#damageItem was a NeoForge extension; ItemStack#hurtAndBreak takes the raw amount and needs a
            // ServerPlayer (or null) rather than an arbitrary LivingEntity.
            stack.hurtAndBreak(amount, (ServerLevel) entityIn.level(), entityIn instanceof final net.minecraft.server.level.ServerPlayer p ? p : null, onBroken);
    ...
    public ItemStack insertItem(final int slot, @NotNull final ItemStack stack, final boolean simulate)
    {
        ...
        if (inSlot.isEmpty())
        {
            if (!simulate)
            {
                ...
                {
                    final int remainder = copy.getCount() - copy.getMaxStackSize();
    ...
    public void write(@NotNull final HolderLookup.Provider provider, final CompoundTag nbtTagCompound)
    {
        ...
        for (int i = 0; i < this.mainInventory.size(); ++i)
        {
            if (!(this.mainInventory.get(i)).isEmpty())
            {
                ...
                compoundNBT.put(NbtTagConstants.STACK, Utils.serializeCodecMess(ItemStack.OPTIONAL_CODEC, provider, this.mainInventory.get(i)));
        ...
        for (int i = 0; i < this.armorInventory.size(); ++i)
        {
            if (!(this.armorInventory.get(i)).isEmpty())
            {
                final CompoundTag compoundNBT = new CompoundTag();
                compoundNBT.putByte(NbtTagConstants.SLOT, (byte) i);
                compoundNBT.put(NbtTagConstants.STACK, Utils.serializeCodecMess(ItemStack.OPTIONAL_CODEC, provider, this.armorInventory.get(i)));
    ...
    public void read(@NotNull final HolderLookup.Provider provider, final CompoundTag nbtTagCompound)
    {
        ...
        for (int i = 0; i < nbtTagList.size(); i++)
        {
            ...
            final ItemStack itemstack = Utils.deserializeCodecMess(ItemStack.OPTIONAL_CODEC, provider, compoundNBT.getCompoundOrEmpty(NbtTagConstants.STACK));
