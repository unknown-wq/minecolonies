...
public class ContainerCrafting extends AbstractContainerMenu
{
    ...
    public void slotsChanged(final Container inventoryIn)
    {
        if (!world.isClientSide())
        {
            ...
            final List<RecipeHolder<CraftingRecipe>> recipes = ((ServerLevel) world).recipeAccess().getRecipes().stream()
                    .filter(holder -> holder.value().getType() == RecipeType.CRAFTING)
                    .map(holder -> (RecipeHolder<CraftingRecipe>) (RecipeHolder<?>) holder)
                    .filter(holder -> holder.value().matches(craftMatrix.asCraftInput(), world))
                    .filter(recipe -> recipe.value().isSpecial()
                            || !((ServerLevel) world).getGameRules().get(GameRules.LIMITED_CRAFTING)
            ...
            else
            {
                ...
                final ItemStack stack = recipes.get(this.recipeIndexSlot.get()).value()
                        .assemble(this.craftMatrix.asCraftInput());
