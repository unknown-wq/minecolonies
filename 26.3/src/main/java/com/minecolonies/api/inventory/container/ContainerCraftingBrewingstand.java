...
import com.minecolonies.api.inventory.api.SlotItemHandler;
...
public class ContainerCraftingBrewingstand extends AbstractContainerMenu
{
    ...
    public ContainerCraftingBrewingstand(final int windowId, final Inventory inv, final BlockPos pos, final int moduleId)
    {
        ...
        this.brewingStandInventory = new IItemHandlerModifiable()
        {
            ...
            public boolean isItemValid(final int slot, @NotNull final ItemStack stack)
            {
                if (slot == 3)
                {
                    return isBrewingReagent(stack);
                }
                else if (slot >= 0 && slot < 3)
                {
                    return isBrewingPotionInput(stack);
                }
                else
                {
                    return false;
    ...
    public ItemStack quickMoveStack(final Player playerIn, final int index)
    {
        ...
        if (slot != null && slot.hasItem())
        {
            ...
            if (isBrewingReagent(stack))
            {
                setInput(stack);
                return ItemStack.EMPTY;
            }
            else if (isBrewingPotionInput(stack) && stack.getCount() == 1)
            {
                setContainer(stack);
                return ItemStack.EMPTY;
            }
    ...
    /**
     * Whether the stack belongs in the top (reagent) slot of a brewing stand.
     * <p>
     * 26.3: {@code PotionBrewing} is gone; brewing is a recipe type now and the accepted inputs are
     * published as recipe property sets, exactly as {@code BrewingStandBlockEntity#canPlaceItem} reads them.
     *
     * @param stack the stack to test.
     * @return true when it is a brewing reagent.
     */
    private boolean isBrewingReagent(@NotNull final ItemStack stack)
    {
        return getWorldObj().recipeAccess().propertySet(RecipePropertySet.BREWING_REAGENTS).test(stack);
    }

    /**
     * Whether the stack belongs in one of the three bottle slots of a brewing stand.
     *
     * @param stack the stack to test.
     * @return true when it is a brewable bottle.
     */
    private boolean isBrewingPotionInput(@NotNull final ItemStack stack)
    {
        return PotionIngredient.isPotionInput(stack, getWorldObj().recipeAccess());
    }
