...
public class RecipeCraftingType<C extends RecipeInput, T extends Recipe<C>> extends CraftingType
{
                                            ...
                                            @NotNull final Level world)
    {
        ...
        for (final RecipeHolder<T> recipe : recipeManager.getRecipes().stream()
                                              .filter(holder -> holder.value().getType() == recipeType)
                                              .map(holder -> (RecipeHolder<T>) (RecipeHolder<?>) holder)
                                              .toList())
        {
                                               ...
                                               @NotNull final Level world)
    {
        ...
        if (recipe.isSpecial()) return;     // invalid or special recipes
        try
        {
