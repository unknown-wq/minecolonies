...
public class GenericRecipe implements IGenericRecipe
{
     ...
     */
    /**
     * 26.2 removed {@code Recipe#getResultItem}; the result is only reachable through the recipe's display list
     * ({@code /opt/mc-src/net/minecraft/world/item/crafting/Recipe.java:41}).
     *
     * @param recipe the recipe.
     * @param world  the level, for registry access.
     * @return the (first) result stack, or empty.
     */
    private static ItemStack resultOf(@NotNull final Recipe<?> recipe, @NotNull final Level world)
    {
        return recipe.display().stream()
                 .map(display -> display.result().resolveForFirstStack(
                   net.minecraft.world.item.crafting.display.SlotDisplayContext.fromLevel(world)))
    ...
    public static IGenericRecipe of(@Nullable final RecipeHolder<?> holder, @NotNull final Level world)
    {
        ...
        // 26.2: Recipe#getIngredients / #getResultItem / #canCraftInDimensions are gone. Ingredients now live in
        // Recipe#placementInfo(), an Ingredient yields Holders rather than stacks, RecipeHolder#id() is a ResourceKey,
        // and the result is only described through the recipe's display list.
        final List<List<ItemStack>> inputs = compactInputs(recipe.placementInfo().ingredients().stream()
                .map(ingredient -> ingredient.items().map(ItemStack::new).toList())
                .toList());

        final Builder builder = builder()
                .withRecipeId(holder.id().identifier())
                .withOutput(resultOf(recipe, world))
        ...
        {
            builder.withGridSize(recipe.placementInfo().ingredients().size() <= 4 ? 2 : 3);
        }
                                                             ...
                                                             @Nullable final Level world)
    {
        ...
        {
            final List<Ingredient> inputs = recipe.placementInfo().ingredients();
            ...
            {
                final List<ItemStack> stacks = inputs.get(slot).items().map(ItemStack::new).toList();
                if (!stacks.isEmpty())
                {
                    inv.setItem(slot, stacks.get(0).copy());
                }
            }
