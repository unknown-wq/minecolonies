...
{
    /**
     * Registers every entity type.
     * <p>
     * Port note (contract C5): {@code RegisterEvent} is gone; the mod entry point calls this directly against
     * {@link BuiltInRegistries#ENTITY_TYPE}.
     */
    public static void init()
    {
        {
            final Registry<EntityType<?>> registry = BuiltInRegistries.ENTITY_TYPE;
            ...
            // Dimensions and eye height copied from vanilla's boats (EntityTypes: sized(1.375F, 0.5625F)
            // .eyeHeight(0.5625F)) -- AbstractBoat's buoyancy maths is written around that hull size.
            ModEntities.BOAT = build(registry, "mcboat",
              EntityType.Builder.of(MinecoloniesBoat::new, MobCategory.MISC)
                .clientTrackingRange(ENTITY_TRACKING_RANGE)
                .updateInterval(ENTITY_UPDATE_FREQUENCY)
    ...
    /**
     * Builds and registers one entity type.
     * <p>
     * Port note: 26.2 {@code EntityType.Builder#build} takes the {@link ResourceKey} instead of a string, and
     * {@code setTrackingRange} / {@code setUpdateInterval} were renamed to {@code clientTrackingRange} /
     * {@code updateInterval}. {@code setShouldReceiveVelocityUpdates} has no counterpart at all -- velocity is
     * always synced now -- so those calls were dropped.
     */
    private static <T extends Entity> EntityType<T> build(final Registry<EntityType<?>> registry, final String key, final EntityType.Builder<T> builder)
    {
        final ResourceKey<EntityType<?>> id =
          ResourceKey.create(Registries.ENTITY_TYPE, Identifier.fromNamespaceAndPath(Constants.MOD_ID, key));
        final EntityType<T> entity = builder.build(id);
        Registry.register(registry, id, entity);
        return entity;
    }
}
