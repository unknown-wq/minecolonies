...
public class InteractionValidatorInitializer
{
    ...
    public static void init()
    {
        ...
        // Stays on the courier's list exactly as long as the warehouse it names is still refusing everything, which is
        // the point of raising it: a chat line scrolls away in seconds, and the state it describes lasts until the
        // player does something about it.
        InteractionValidatorRegistry.registerPosBasedPredicate(Component.translatableEscape(COM_MINECOLONIES_COREMOD_JOB_DELIVERYMAN_WAREHOUSEFULL),
          (cit, pos) -> {
              if (!(cit.getJob() instanceof JobDeliveryman) || cit.getColony() == null)
              {
                  return false;
              }
              final IBuilding building = cit.getColony().getServerBuildingManager().getBuilding(pos);
              return building instanceof IWareHouse
                       && ((IWareHouse) building).getTileEntity() instanceof final TileEntityWareHouse warehouse
                       && warehouse.isFull();
          });
        ...
        // The shaft has stopped going down. The complaint stands exactly as long as the building says the stall
        // does: the watchdog clears it the moment the bottom of the ladder moves, so the message clears itself
        // when the shaft comes back to life without the player having to dismiss it.
        InteractionValidatorRegistry.registerPosBasedPredicate(Component.translatableEscape(MINER_SHAFT_STUCK),
          (citizen, pos) -> citizen.getWorkBuilding() instanceof final BuildingMiner miner && miner.getShaftStallTicks() > 0);

        // The miner is being hurt where he is working. Valid while he is still burning or still hurt; once he has
        // healed up in the hut the complaint is stale and goes away on its own.
        InteractionValidatorRegistry.registerPosBasedPredicate(Component.translatableEscape(MINER_IN_DANGER),
          (citizen, pos) -> citizen.getJob() instanceof JobMiner
                              && citizen.getEntity()
                                   .map(entity -> entity.getRemainingFireTicks() > 0 || entity.getHealth() < entity.getMaxHealth() * 0.75f)
                                   .orElse(false));
        ...
        // The free mode clause matches EntityAIWorkBeekeeper#getFlowers, which answers the question for the player
        // rather than asking it. Without it here too, an interaction raised before the switch was thrown would never
        // be revalidated away and the nag would outlive the condition.
        InteractionValidatorRegistry.registerStandardPredicate(Component.translatable(COM_MINECOLONIES_COREMOD_BEEKEEPER_NOFLOWERS),
          citizen -> citizen.getWorkBuilding() instanceof BuildingBeekeeper
                       && !FreeMode.isOn(citizen.getWorkBuilding())
