...
public class StandardRetryingRequestResolver implements IRetryingRequestResolver
{
    ...
    {
        // Every real resolver - the requesting hut's own racks, the warehouses, the crafters - sits at a higher
        // priority and has already declined by the time a request lands here, so this is the one place in the mod
        // that sees "nobody in the colony can serve this". Free mode conjures it instead of scheduling a retry.
        if (FreeMode.fulfil(manager, request))
        {
            return;
        }
