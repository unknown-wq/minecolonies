    ...
    public void onServerResponseTriggered(final int responseId, final Player player, final ICitizenData data)
    {
        ...
        if (response.equals(recruitAnswer.getA()) && data instanceof IVisitorData)
        {
            ...
            if (colony.getCitizenManager().getCurrentCitizenCount() < colony.getCitizenManager().getPotentialMaxCitizens())
            {
                ...
                {
                    // 26.2/Fabric: the tavern this visitor was actually living in, read before setHomeBuilding(null)
                    // clears it, rather than whichever tavern the building map yielded first. With several taverns the
                    // recruitment statistics all landed on one of them, so the player could not see which tavern was
                    // earning its keep. Falls back to the nearest tavern to where the visitor is standing for a
                    // visitor that somehow has no home, and the stat helpers already tolerate null.
                    IBuilding tavern = data.getHomeBuilding();
                    if (tavern == null || tavern.getBuildingType() != ModBuildings.tavern.get())
                    {
                        tavern = colony.getServerBuildingManager()
                          .getClosestBuildingMatching(data.getLastPosition(), b -> b.getBuildingType() == ModBuildings.tavern.get());
                    }
