...
public class JobChickenHerder extends AbstractJob<EntityAIWorkChickenHerder, JobChickenHerder>
{
    ...
    {
        // #minecraft:eggs, not Items.EGG: 26.2 chickens lay by variant, so a warm chicken drops a brown egg and a
        // cold one a blue egg. Matching only the white egg meant the herder handed those straight back to the
        // player instead of ever keeping one.
        if (pickedUpStack.is(Items.FEATHER) || pickedUpStack.is(ItemTags.EGGS))
        {
