...
public final class ColonyView implements IColonyView
{
    ...
    /**
     * Whether free mode is on for this colony. See {@link com.minecolonies.core.debug.FreeMode}.
     * <p>
     * The behaviour half of free mode is server side only and deliberately reads nothing on the client. This copy
     * exists for one reason: the scarecrow window has to know which farm field sizes the server will accept, so that
     * an oversized field is refused before it is sent rather than snapping back afterwards.
     */
     ...
     * <p>
     * Synced because the client is where it matters most: the border renderer resolves a chunk's owning colony to a
     * view and asks it what colour to draw, and enemy ground has to come out red whatever the player's team colour
     * setting says.
     */
    ...
    public static void serializeNetworkData(@NotNull Colony colony, @NotNull RegistryFriendlyByteBuf buf, boolean hasNewSubscribers)
    {
        ...
        buf.writeBoolean(colony.isFreeMode());
        buf.writeBoolean(colony.isHostile());
    ...
    {
        // Read off the citizen views rather than kept as a flag: the server never sends one, and the only client side
        // caller would be a GUI already walking this list anyway.
        for (final ICitizenDataView citizen : citizens.values())
        {
            if (citizen.isChild())
            {
                return true;
