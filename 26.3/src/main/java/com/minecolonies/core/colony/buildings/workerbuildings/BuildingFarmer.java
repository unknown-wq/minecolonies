...
public class BuildingFarmer extends AbstractBuilding
{
    ...
    public Map<Predicate<ItemStack>, Tuple<Integer, Boolean>> getRequiredItemsAndAmount()
    {
        ...
        for (BuildingExtensionsModule module : getModulesByType(BuildingExtensionsModule.class))
        {
            for (final IBuildingExtension extension : module.getOwnedExtensions())
            {
                ...
                {
                    // Every seed of a mixed field, not only its first: a seed that is not kept back is dumped into
                    // the racks at the end of the pass and has to be fetched again for the next one.
                    for (final ItemStack seed : farmField.getSeeds())
                    {
                        final ItemStack kept = seed.copy();
                        toKeep.put(stack -> ItemStack.isSameItem(kept, stack), new Tuple<>(64, true));
                    }
                }
            }
        }
    ...
    public boolean canEat(final ItemStack stack)
    {
        for (BuildingExtensionsModule module : getModulesByType(BuildingExtensionsModule.class))
        {
            for (final IBuildingExtension field : module.getOwnedExtensions())
            {
                ...
                {
                    // Any of the field's seeds, so a mixed field's potatoes are no more edible than its wheat.
                    for (final ItemStack seed : farmField.getSeeds())
                    {
                        if (ItemStackUtils.compareItemStacksIgnoreStackSize(seed, stack))
                        {
                            return false;
                        }
                    }
    ...
    public static class CraftingModule extends AbstractCraftingBuildingModule.Crafting
    {
        ...
        public List<IGenericRecipe> getAdditionalRecipesForDisplayPurposesOnly(@NotNull Level world)
        {
            ...
            for (final ItemStack stack : IColonyManager.getInstance().getCompatibilityManager().getListOfAllItems())
            {
                ...
                else if (stack.getItem() instanceof BlockItem item && item.getBlock() instanceof CropBlock crop)
                {
                    // regular crop
                    recipes.add(GenericRecipe.builder()
                            .withInputs(List.of(List.of(crop.defaultBlockState().getCloneItemStack(world, BlockPos.ZERO, false))))
                            .withIntermediate(Blocks.FARMLAND)
                            .withLootTable(crop.getLootTable().orElse(null))
                            .withRequiredTool(ModEquipmentTypes.hoe.get())
                            .build());
                }
                else if (stack.is(ConventionalItemTags.SEEDS))
                {
        ...
        private List<Component> provideBiomeList(@NotNull final TagKey<Biome> preferredBiome)
        {
            ...
            final Registry<Biome> biomeRegistry = mc.level.registryAccess().lookupOrThrow(preferredBiome.registry());
