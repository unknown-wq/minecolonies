...
public abstract class LivingBuildingView extends AbstractBuildingView
{
    ...
    /**
     * How many of this house's beds are taken, which is not the same as how many people live here.
     * <p>
     * 26.2/Fabric: the client half of {@code LivingBuildingModule#getResidentCount()}. A child lives with its parents
     * and holds no bed, so it is in {@link #getResidents()} -- the player should see the whole household -- but it
     * does not count towards the level. Every "is this house full" question on the client has to use this or the GUI
     * disagrees with the server about whether a citizen can be moved in.
     *
    ...
    {
        int adults = 0;
        for (final int id : getResidents())
        {
            final var resident = getColony().getCitizen(id);
            if (resident == null || !resident.isChild())
            {
                adults++;
            }
        }
