...
public final class ShipBasedRaiderUtils
{
    ...
    public static boolean canPlaceShipAt(final BlockPos pos, final Blueprint ship, final Level world, final int neededDepth)
    {
        ...
        allowedShipMaterials.add(state -> !PathfindingUtils.blocksMotion(state) && state.getFluidState().is(FluidTags.WATER));
        ...
        for (int x = 0; x < xDist; x++)
        {
            for (int z = 0; z < zDist; z++)
            {
                ...
                if (suitableBlock)
                {
                    for (int i = 1; i <= 5; i++)
                    {
                        if (PathfindingUtils.blocksMotion(world.getBlockState(new BlockPos(from.getX() + (x * xDir), baseY + i, from.getZ() + (z * zDir)))))
                        {
                            suitableBlock = false;
                            break;
                        }
      ...
      final boolean underWater)
    {
        ...
        if (WorldUtil.isBlockLoaded(colony.getWorld(), startPos))
        {
            if (underWater)
            {
                return BlockPosUtil.findAround(colony.getWorld(),
                  ...
                  30,
                  3,
                  (world, pos) -> (world.getBlockState(pos).isSolid() || world.getBlockState(pos).liquid())
                                    && !PathfindingUtils.blocksMotion(world.getBlockState(pos.above()))
                                    && !PathfindingUtils.blocksMotion(world.getBlockState(pos.above(2))));
            }
            else
            {
                return BlockPosUtil.findAround(colony.getWorld(),
        ...
        // 26.3: BlockPos has no copy constructor any more. It is immutable, and startPos is reassigned
        // through offset() below rather than mutated, so the copy was never needed.
    ...
    public static BlockPos findSpawnPosOnShip(final BlockPos spawnPos, final Level world, final int radius)
    {
        for (int y = 0; y <= radius * 2; y += 2)
        {
            for (int x = -radius; x <= radius; x++)
            {
                ...
                {
                    if (!PathfindingUtils.blocksMotion(world.getBlockState(spawnPos.offset(x, y, z))) && !PathfindingUtils.blocksMotion(world.getBlockState(spawnPos.offset(x, y + 1, z))))
                    {
