      ...
      @NotNull final IRecipeStorage storage)
    {
        ...
        for (final ItemStorage ingredient : storage.getCleanedInput())
        {
            if (!ItemStackUtils.isEmpty(ingredient.getItemStack()))
            {
                ...
                // PORT(26.2): ItemStack#getCraftingRemainingItem is gone; the remainder is an Item-level template now.
            final ItemStackTemplate remainder = ingredient.getItemStack().getItem().getCraftingRemainder();
