...
public class BuildRequestMessage extends AbstractBuildingServerMessage<IBuilding>
{
    ...
    /**
     * The level to build to, or 0 for the next one. Only free mode ever sends anything else, and the server checks that
    ...
    {
        this(building, mode, builder, 0);
    }

    /**
     * Creates a build request that names the level to build to.
     *
     * @param building    the building we're executing on.
     * @param mode        Mode of the request, 1 is repair, 0 is build.
     * @param builder     the builder we're assinging the request to
     * @param targetLevel the level to build to, 0 for the next one.
     */
    public BuildRequestMessage(@NotNull final IBuildingView building, final Mode mode, final BlockPos builder, final int targetLevel)
    {
        super(TYPE, building);
        this.mode = mode;
        ...
        else
        {
            switch (mode)
            {
                case BUILD:
                    if (targetLevel > 0)
                    {
                        building.requestUpgradeTo(player, builder, targetLevel);
                    }
                    else
                    {
