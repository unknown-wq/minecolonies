...
{
    public static final EnumProperty<net.minecraft.core.Direction> HORIZONTAL_FACING = HorizontalDirectionalBlock.FACING;
    ...
    {
        this(DyeColor.WHITE,
            IBlockMinecolonies.withBlockId(Properties.of().mapColor(MapColor.WOOD)
              .sound(SoundType.WOOD)
                .noCollision()
                .strength(1F)
                .sound(SoundType.WOOD), REGISTRY_NAME_WALL));
    }
    ...
    // 26.2: BlockBehaviour#getDescriptionId is final -- the description id comes from the block's registry key
    // now and cannot be redirected to the item's. Original body: return this.asItem().getDescriptionId();
