...
public class DiseasesListener extends SimpleJsonResourceReloadListener<JsonElement>
{
     ...
     * The map of diseases.
     */
    // 26.2: WeightedRandomList<T extends WeightedEntry> became WeightedList<T> with the weight held by a
    // separate Weighted<T> wrapper; the rarity that used to come from Disease#getWeight() is applied here.
    private static WeightedList<Disease> DISEASES = WeightedList.of();
    ...
    {
        super(DataListenerUtils.JSON_CODEC, DataListenerUtils.dir("colony/diseases"));
    }
    ...
    public static void sendGlobalDiseasesPackets(final ServerPlayer player)
    {
        ...
        for (final Disease disease : getDiseases())
        {
    ...
    {
        return DISEASES.unwrap().stream().map(Weighted::value).toList();
    }
    ...
    /**
     * Wrap each disease in a {@link Weighted} carrying its rarity and build the pick list.
     *
     * @param diseases the diseases to weigh.
     * @return the weighted list.
     */
    @NotNull
    private static WeightedList<Disease> weigh(final List<Disease> diseases)
    {
        final WeightedList.Builder<Disease> builder = WeightedList.builder();
        for (final Disease disease : diseases)
        {
            builder.add(disease, Math.max(1, disease.rarity()));
        }
      ...
      final @NotNull ProfilerFiller profiler)
    {
        ...
        for (final Map.Entry<Identifier, JsonElement> entry : jsonElementMap.entrySet())
        {
            ...
            for (final JsonElement jsonElement : object.getAsJsonArray(KEY_ITEMS))
            {
                ...
                final ItemStorage cureItem = new ItemStorage(DataListenerUtils.registryLookup(), jsonElement.getAsJsonObject());
