...
public class TileEntityColonyFlagRenderer implements BlockEntityRenderer<TileEntityColonyFlag, TileEntityColonyFlagRenderer.ColonyFlagRenderState>
{
      ...
      @NotNull final CameraRenderState camera)
    {
        ...
        // 26.3: neither submitModel nor BannerRenderer#submitPatterns takes the block-breaking overlay any
        // more; it is submitted separately, at an order past the last pattern layer, exactly as vanilla's
        // BannerRenderer#submitBanner does it (BannerRenderer.java:170-177).
        if (state.breakProgress != null)
        {
            final int overlayOrder = state.patterns.layers().size() + 2;
            submitNodeCollector.order(overlayOrder)
              .submitCrumblingOverlay(poleModel,
                Unit.INSTANCE,
                poseStack,
                Sheets.BANNER_BASE.renderType(poleModel.renderType()),
                state.lightCoords,
                OverlayTexture.NO_OVERLAY,
                -1,
                state.breakProgress);
            submitNodeCollector.order(overlayOrder)
              .submitCrumblingOverlay(flagModel,
                state.phase,
                poseStack,
                Sheets.BANNER_BASE.renderType(flagModel.renderType()),
                state.lightCoords,
                OverlayTexture.NO_OVERLAY,
                -1,
                state.breakProgress);
