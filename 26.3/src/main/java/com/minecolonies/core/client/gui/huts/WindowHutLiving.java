...
import static com.minecolonies.api.util.constant.TranslationConstants.LABEL_HOUSE_ASSIGNED_CITIZENS_WITH_CHILDREN;
...
public class WindowHutLiving extends AbstractBuildingMainWindow<LivingBuildingView>
{
    ...
    {
        // The first number is beds taken, not people in the house: a child lives with its parents and holds no bed, so
        // counting it here would show "3/2" for a house that is not full and make the colony's bed arithmetic look
        // broken. The children are named separately instead, so the household still adds up on screen.
        final int beds = buildingView.getOccupiedBeds();
        final int children = buildingView.getResidents().size() - beds;
        findPaneOfTypeByID(ASSIGNED_LABEL, Text.class).setText(children == 0
                                                                 ? Component.translatableEscape(LABEL_HOUSE_ASSIGNED_CITIZENS, beds, buildingView.getMax())
                                                                 : Component.translatableEscape(LABEL_HOUSE_ASSIGNED_CITIZENS_WITH_CHILDREN,
                                                                   beds,
                                                                   buildingView.getMax(),
                                                                   children));
