...
// PORT-26.2: Structurize moved its Tuple to com.ldtteam.structurize.api and getInWorldCorners returns that one.
...
public class ColonyBlueprintRenderer
{
    ...
    static void renderBlueprints(final WorldEventContext ctx)
    {
        if (ModKeyMappings.TOGGLE_GOGGLES.get().consumeClick())
        {
            ...
            // PORT-26.2: Player#playNotifySound is gone; the client-side equivalent is Level#playLocalSound at the player.
            ctx.clientLevel.playLocalSound(ctx.clientPlayer.blockPosition(), SoundEvents.NOTE_BLOCK_PLING.value(), SoundSource.NEUTRAL,
              1.0F, shouldRenderBlueprints ? 0.75F : 0.25F, false);
        }
