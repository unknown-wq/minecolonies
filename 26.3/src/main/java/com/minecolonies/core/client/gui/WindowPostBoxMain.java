    ...
    private void updateResources()
    {
        final Predicate<ItemStack> filterPredicate =
            stack -> filter.isEmpty() || stack.getItem().getDescriptionId().toLowerCase(Locale.US).contains(filter.toLowerCase(Locale.US)) || stack.getHoverName()
                ...
                .contains(filter.toLowerCase(Locale.US)) || (stack.is(Items.ENCHANTED_BOOK) && stack.getOrDefault(DataComponents.STORED_ENCHANTMENTS, net.minecraft.world.item.enchantment.ItemEnchantments.EMPTY)
                .entrySet()
                .stream()
