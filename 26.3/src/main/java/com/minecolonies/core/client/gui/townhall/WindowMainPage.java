...
public class WindowMainPage extends AbstractWindowTownHall
{
    ...
    private void initDropDowns()
    {
        ...
        final List<ChatFormatting> textColors = // PORT-26.2: ChatFormatting lost isColor()/getName(); the colour set is what TextColor maps.
          Arrays.stream(ChatFormatting.values()).filter(f -> TextColor.fromLegacyFormat(f) != null).toList();
        ...
        colorDropDownList.setDataProvider(new DropDownList.DataProvider()
        {
            ...
            public MutableComponent getLabel(final int index)
            {
                ...
                {
                    final String colorName = textColors.get(index).name().toLowerCase(Locale.US).replace("_", " ");
    ...
    private void openBannerPicker(@NotNull final Button button)
    {
        ...
        Minecraft.getInstance().gui.setScreen(window);
    }
