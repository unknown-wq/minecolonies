...
public class RequestWindowCitizen extends AbstractWindowCitizen
{
    ...
    private static class CitizenRequestTreeWindowModule extends RequestTreeWindowModule
    {
        ...
        {
            if (buildingView == null || !(request.getRequest() instanceof IDeliverable deliverable))
            {
                return false;
            }

            // A child request belongs to whichever hut raised it, which is not necessarily the one this citizen
            // works in. Handing it over from here credited the wrong building and left the real one still waiting.
