...
public class CommandDeleteColony implements IMCColonyOfficerCommand
{
    ...
    {
        if (!IMCCommand.hasOpPermission(context.getSource()) && !MineColonies.getConfig().getServer().canPlayerUseDeleteColonyCommand.get())
        {
    ...
    {
        // Registered from ModArgumentTypes, not from a static block here -- see the note there.
