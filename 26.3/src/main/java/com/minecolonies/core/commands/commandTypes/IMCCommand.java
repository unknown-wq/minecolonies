...
public interface IMCCommand
{
    ...
    /**
     * 26.2 replaced numeric command permission levels with the {@code net.minecraft.server.permissions}
     * {@code Permission}/{@code PermissionSet} model. {@code hasPermission(4)} -- the old "owner/level 4"
     * check -- is now {@code Permissions.COMMANDS_OWNER}.
     *
     * @param source the command source.
     * @return whether the source has full operator rights.
     */
    static boolean hasOpPermission(final CommandSourceStack source)
    {
        return source.permissions().hasPermission(net.minecraft.server.permissions.Permissions.COMMANDS_OWNER);
    }
    ...
    {
        return context.getSource().getEntity() instanceof Player || IMCCommand.hasOpPermission(context.getSource());
    }
