 ...
 */
public class ColonyStoryListener extends SimpleJsonResourceReloadListener<JsonElement> implements IdentifiableResourceReloadListener
{
    /**
     * Gson instance
     */
    ...
    /**
     * Registry id of this reload listener; Fabric requires one, NeoForge did not.
     */
    private static final Identifier LISTENER_ID = Identifier.fromNamespaceAndPath(MOD_ID, "colony_stories");

    /**
     * Installs the listener on the client resource pipeline. Called once from the client initializer.
     * <p>
     * Port note (contract C5): was {@code @SubscribeEvent modInitClient(RegisterClientReloadListenersEvent)}.
     */
    @Environment(EnvType.CLIENT)
    public static void register()
    {
        ResourceManagerHelper.get(PackType.CLIENT_RESOURCES).registerReloadListener(new ColonyStoryListener());
    }
    ...
    {
        // 26.2 turned SimpleJsonResourceReloadListener into a codec-driven, generic class; ExtraCodecs.JSON is
        // the identity codec that gives back the raw JsonElement this listener has always parsed by hand.
