...
public class CitizenItemUtils
{
    ...
    public static void hitBlockWithToolInHand(@NotNull final AbstractEntityCitizen citizen, @Nullable final BlockPos blockPos, final boolean breakBlock)
    {
        ...
        // 26.3: LivingEntity#swing(InteractionHand) is gone -- the animation now comes from the held item as a
        // SwingAnimation component rather than being implied. Mob#swingForAttack does exactly that lookup
        // (Mob.java:1469-1472) and falls back to SwingAnimation.DEFAULT, which is the animation the old
        // one-argument call always played. Written this way rather than spelled out longhand so that all 49
        // swing sites in the mod read identically.
        citizen.swingForAttack(citizen.getUsedItemHand());
    ...
    {
        if (!(citizen.level() instanceof net.minecraft.server.level.ServerLevel serverLevel))
        {
            return null;
        }
        return citizen.spawnAtLocation(serverLevel, itemstack, 0.0F);
    }
    ...
    public static void updateArmorDamage(@NotNull final AbstractEntityCitizen citizen, final double damage)
    {
        ...
        for (EquipmentSlot equipmentSlot : EquipmentSlot.values())
        {
            if (equipmentSlot.getType() == EquipmentSlot.Type.HUMANOID_ARMOR)
            {
                ...
                equipment.hurtAndBreak(armorDmg, (ServerLevel) citizen.level(), null, (s) -> {
    ...
    public static double applyMending(@NotNull final AbstractEntityCitizen citizen, final double xp)
    {
        ...
        for (final EquipmentSlot equipmentSlot : EquipmentSlot.values())
        {
            ...
            if (!ItemStackUtils.isEmpty(tool) && tool.isDamaged() && tool.isEnchanted() && EnchantmentHelper.getItemEnchantmentLevel(Utils.getRegistryValue(Enchantments.MENDING, citizen.level()), tool) > 0)
            {
                //2 xp to heal 1 dmg
