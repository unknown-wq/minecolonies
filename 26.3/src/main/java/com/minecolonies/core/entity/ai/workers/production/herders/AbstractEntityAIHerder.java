...
public abstract class AbstractEntityAIHerder<J extends AbstractJob<?, J>, B extends AbstractBuilding> extends AbstractEntityAIInteract<J, B>
{
     ...
     * <p>
     * Public because the free mode stocking pass in {@code AnimalPen} has to fill a hut to the same ceiling the
     * worker culls it back to, and it has no AI instance to ask. Reading the constant rather than copying the number
     * keeps the two in step; {@link #getMaxAnimalMultiplier()} stays the override point and no herder overrides it
     * today, so the constant is the ceiling for all six huts.
     */
    public static final int ANIMAL_MULTIPLIER = 2;
    ...
    public boolean equipTool(final InteractionHand hand, final EquipmentTypeEntry toolType)
    {
        ...
        {
            // No tool of this kind and none will be asked for, so report the hand as ready and let the caller work
            // bare handed. Shearing generates the wool itself and butchering deals a flat constant of damage
            // (getButcheringAttackDamage), so neither outcome depends on what is actually held.
            CitizenItemUtils.removeHeldItem(worker);
            return true;
        }
    ...
    {
        worker.swingForAttack(InteractionHand.MAIN_HAND); // visual only
    ...
    {
        if (worksWithoutTools() && ItemStackUtils.isEmpty(worker.getMainHandItem()))
        {
            // Bare handed because no weapon was requested. The blow below deals getButcheringAttackDamage() no matter
            // what is held, so the animal dies just the same; only the looting branch is skipped, since there is no
            // weapon to mirror onto the fake player.
            if (animal != null && !walkingToAnimal(animal))
            {
