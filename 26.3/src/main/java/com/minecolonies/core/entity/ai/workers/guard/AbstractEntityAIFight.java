...
public abstract class AbstractEntityAIFight<J extends AbstractJobGuard<J>, B extends AbstractBuildingGuards> extends AbstractEntityAIInteract<J, B>
{
    ...
    /**
     * The equipment types this guard wants in hand right now.
     * <p>
     * Defaults to the fixed {@link #toolsNeeded} list every guard AI fills in its constructor. It is overridable
     * because a guard's weapon is not always fixed for the life of the job: a knight fields a spear instead of a
     * sword whenever his hut can arm him with one (see {@code EntityAIMelee#getToolsNeeded}). Whatever this returns
     * is what {@link #prepare()} both requests through the request system and puts in the worker's hand, so
     * overriding it is enough to move a guard onto a different weapon end to end.
     *
     * @return the list of equipment types to request and equip.
     */
    ...
    {
        for (final EquipmentTypeEntry tool : getToolsNeeded())
        {
            ...
            InventoryFunctions.matchFirstInProviderWithSimpleAction(worker,
              stack -> !ItemStackUtils.isEmpty(stack)
                         ...
                         && ItemStackUtils.hasEquipmentLevel(stack, tool, ItemStackUtils.isSpear(stack) ? -1 : 0, building.getMaxEquipmentLevel()),
