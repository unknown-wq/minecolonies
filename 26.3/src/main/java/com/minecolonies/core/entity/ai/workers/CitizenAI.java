...
public class CitizenAI implements IStateAI
{
    ...
    private IState calculateNextState()
    {
        ...
        // Sleeping
        // The night switch (/mc colony workoverride <colony> night on, builders only) skips the whole going-to-bed
        // half of this branch and falls into the waking-up half instead, so a builder that was already asleep when
        // the switch was thrown gets up rather than lying there until dawn. With the switch off - which is how every
        // colony starts - worksThroughTheNight is false and this reads exactly as it always did.
        final boolean worksThroughTheNight =
          WorkOverride.worksThroughTheNight(citizen.getCitizenColonyHandler().getColonyOrRegister(), citizen.getCitizenJobHandler().getColonyJob());
        if (worksThroughTheNight)
        {
            // A builder kept out all night never gets the reset that going to bed would have given it.
            WorkOverride.stayRested(citizen.getCitizenData());
        }

        if (!worksThroughTheNight && !WorldUtil.isPastTime(CompatibilityUtils.getWorldFromCitizen(citizen), NIGHT - 2000))
        {
        ...
        {
            // The mourning switch (/mc colony workoverride <colony> mourning on) lifts the work stoppage only. The
            // citizen is still mourning everywhere else - the grief, the happiness cost, the chat bubble raised just
            // below - it simply does not down tools for it. The status icon is left to the work branch further down
            // so the player can still tell at a glance who is working.
            if (WorkOverride.worksWhileMourning(citizen.getCitizenColonyHandler().getColonyOrRegister()))
            {
                // shouldMourn, i.e. the deceased list is not empty, because the name below is read off that list and
                // burying the grave empties it (CitizenManager#updateCitizenMourn) while isMourning stays true until
                // the next dawn. The unguarded branch below only reaches that read on a state change and so hits the
                // window rarely; this one would walk into it every pass.
                // triggerInteraction ignores an interaction the citizen already has, so this settles after the first
                // pass rather than raising the bubble again every time round.
                if (citizen.getCitizenData().getCitizenMournHandler().shouldMourn())
                {
