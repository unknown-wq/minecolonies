    ...
    private IAIState requestCure()
    {
        ...
        for (final ItemStorage cure : disease.cureItems())
        {
                  ...
                  && InventoryUtils.getCountFromBuilding(building, Disease.hasCureItem(cure)) <= 0)
            {
                ...
                {
                    // Hand over as many as a request would have brought, so the healer keeps a buffer for the next
                    // patient instead of conjuring one cure at a time.
                    supplyMaterialWithoutRequest(cure.getItemStack(), REQUEST_COUNT);
                    continue;
                }
