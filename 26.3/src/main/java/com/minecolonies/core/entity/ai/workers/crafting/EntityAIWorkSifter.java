...
public class EntityAIWorkSifter extends AbstractEntityAICrafting<JobSifter, BuildingSifter>
{
    ...
    protected IAIState sift()
    {
        ...
        if (currentRecipeStorage == null)
        {
            ...
            if (currentRecipeStorage == null && FreeMode.isOn(building) && supplySiftingMaterials(module))
            {
                currentRecipeStorage = module.getFirstFulfillableRecipe(ItemStackUtils::isEmpty, 1, false);
            }
        }
    ...
    /**
     * Stock the hut with the mesh and the sievable block of the first recipe the sifter knows, out of thin air.
     * <p>
     * The sifter is the one worker that never asks for what it is missing - there is no request anywhere in the class,
     * only the SIFTER_NO_MESH interaction and a spin - so free mode cannot reach it through the request system and has
     * to hand the materials over directly. They go into the hut rather than the worker because the sifter deliberately
     * sifts out of the building, see the "we don't want the mesh in our inventory" branch below.
     *
     * @param module the sifter's crafting module.
     * @return true if anything was handed over.
     */
    private boolean supplySiftingMaterials(@NotNull final ICraftingBuildingModule module)
    {
        for (final IToken<?> token : module.getRecipes())
        {
            ...
            {
                supplied |= InventoryUtils.addItemStackToProvider(building, ingredient.getItemStack().copyWithCount(ingredient.getAmount()));
            }
            return supplied;
        }
        return false;
