...
public class BuildingStructureHandler<J extends AbstractJobStructure<?, J>, B extends AbstractBuildingStructureBuilder> extends AbstractStructureHandler
{
    ...
    public void prePlacementLogic(final BlockPos worldPos, final BlockState blockState, final List<ItemStack> requiredItems)
    {
        ...
        structureAI.getWorker().swingForAttack(InteractionHand.MAIN_HAND);
    }

    // 26.2: IStructureHandler#getInventory is declared with Structurize's own IItemHandler copy (NeoForge's is
    // gone and both projects vendored their own). The two are structurally identical, so the citizen inventory
    // crosses over through com.minecolonies.core.util.StructurizeItemHandlerBridge.
    @Nullable
    @Override
    public com.ldtteam.structurize.compat.itemhandler.IItemHandler getInventory()
    {
        return com.minecolonies.core.util.StructurizeItemHandlerBridge.toStructurize(structureAI.getWorker().getInventoryCitizen());
    }
    ...
    {
        if (structureAI.buildsWithoutResources())
        {
            return true;
        }
    ...
    public void consume(final List<ItemStack> requiredItems)
    {
        ...
        if (this.getInventory() != null)
        {
            for (final ItemStack tempStack : requiredItems)
            {
                ...
                {
                    InventoryUtils.reduceBucketAwareStackInItemHandler(
                      com.minecolonies.core.util.StructurizeItemHandlerBridge.fromStructurize(this.getInventory()), tempStack, tempStack.getCount());
                }
            }
        }
    }
