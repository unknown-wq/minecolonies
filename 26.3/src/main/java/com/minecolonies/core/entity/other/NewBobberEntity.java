...
public class NewBobberEntity extends Projectile
{
    ...
    /**
     * Smooths the client's copy of the bobber between the position updates the server sends.
     * <p>
     * PORT-NOTE(26.2): 1.21.1 did the opposite -- it overrode {@code lerpTo} with an empty body to switch client
     * interpolation <em>off</em>, and the port dropped that override because 26.2 has no method with that
     * signature. What 26.2 replaced it with is {@link InterpolationHandler}, and the default for a plain entity is
     * {@code getInterpolation() == null}, which means "snap straight to whatever the server last said" -- the same
     * behaviour the old empty override produced.
     * <p>
     * That is not what vanilla's own bobber does any more. {@code FishingHook} carries a handler, returns it from
     * {@code getInterpolation()} and calls {@code interpolate()} at the top of its tick, so its position glides
     * between server updates instead of stepping to them. Without it the hook lands on each update the moment it
     * arrives while the client keeps simulating it in between, and the line drawn from the fisherman's hand to the
     * hook jerks back and forth - which is exactly what was reported in play.
     * <p>
     * PORT-NOTE(26.3): {@code InterpolationHandler} became an interface and {@code Entity#getInterpolation()} became
     * {@code final}. The handler is now built once by {@code Entity}'s constructor through the overridable
     * {@code createInterpolationHandler()}, so the field and the {@code getInterpolation()} override are gone and
     * {@link #createInterpolationHandler()} below carries the intent instead -- the same shape vanilla
     * {@code FishingHook} uses. {@code interpolate()} is no longer called from {@code tick()} either: the level
     * drives it through {@code Entity#commonTick()} on the client, and calling it a second time would double-step
     * the hook.
     */
    ...
    // PORT-NOTE(26.2): Entity.noCulling is gone. The equivalent knob moved to the renderer:
    // EntityRenderer#affectedByCulling(T) must return false in core/client/render/RenderFishHook,
    // the way vanilla FishingHookRenderer does.

    @Override
    protected InterpolationHandler createInterpolationHandler() {
        // Same as vanilla FishingHook#createInterpolationHandler: LinearInterpolationHandler.create returns NO_OP
        // server-side and a 3-step linear handler on the client.
    ...
    {
        if (citizen.getCitizenColonyHandler().getWorkBuilding() instanceof final AbstractBuilding workBuilding && workBuilding.worksWithoutTools())
        {
            // The hut is set to never request tools, so the fisherman may be casting bare handed. Without this the
            // hook would discard itself on its very first tick for want of a rod and nothing would ever be caught.
            if (!citizen.isRemoved() && citizen.isAlive() && !(this.distanceToSqr(citizen) > 1024.0))
            {
                return false;
            }
        ...
        // Any fishing rod, not only the vanilla one. ModEquipmentTypes#fishing_rod, which is what decides whether
        // the fisherman may work at all, accepts every FishingRodItem -- so a modded rod got him hired and then had
        // his hook discard itself on its first tick, with nothing said anywhere.
    ...
    @Override
    protected void addAdditionalSaveData(@NotNull final ValueOutput output)
    {

    }

    @Override
    protected void readAdditionalSaveData(@NotNull final ValueInput input)
    {
        ...
        if (!this.level().isClientSide() && citizen != null && !this.shouldStopFishing(citizen))
        {
            ...
            else if (this.nibble > 0)
            {
                LootParams lootparams = new LootParams.Builder((ServerLevel) this.level())
                                          ...
                                          // ORIGIN + required TOOL + optional THIS_ENTITY, and ContextMap.Builder
                                          // throws on anything else rather than ignoring it, which paused the
                                          // fisherman on every catch. ResearchUnlocked read the citizen from this
                                          // parameter; it falls back through THIS_ENTITY (the bobber, not a citizen)
                                          // to ORIGIN, which resolves the colony from the position the bobber is
                                          // floating in — inside the fisherman's own colony.
