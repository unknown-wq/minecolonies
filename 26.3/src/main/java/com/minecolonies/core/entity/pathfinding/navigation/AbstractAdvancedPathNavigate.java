...
public abstract class AbstractAdvancedPathNavigate extends GroundPathNavigation
{
    ...
    /**
     * The position the last path job was asked to reach, whether or not it found a way there.
     * <p>
     * Unlike {@code getSafeDestination()} this is set for every path job, not only the ones asking for a safe
     * destination, so it is still there when the pathfinder came back with nothing. That is exactly the case a caller
     * has to recognise: a destination that produces no path leaves no other trace.
     *
     * @return the requested destination, or null if nothing has been requested yet.
     */
    @Nullable
