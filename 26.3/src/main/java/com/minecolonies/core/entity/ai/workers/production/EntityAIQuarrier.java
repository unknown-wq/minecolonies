 ...
 */
// PORT-NOTE(26.2): CHECKED, NO AI BEHAVIOUR AFFECTED. Same case as EntityAIStructureMiner — see the note there.
// Nothing in this class is disabled; the two canPerformAction(PICKAXE_DIG / SHOVEL_DIG) tests in getRenderMeta
// became stack.is(ItemTags.PICKAXES / SHOVELS), and their result only ever reaches the renderData string that
// picks the tool icon on the citizen's belt.
        ...
        switch (currState)
        {
            case SOLID:
                result = placer.executeStructureStep(world,
                  ...
                  () -> placer.getIterator()
                          .decrement(orPredicate(DONT_TOUCH_PREDICATE, (info, pos, handler) -> !BlockUtils.isAnySolid(info.getBlockInfo().getState())
