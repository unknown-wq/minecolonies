...
public class EntityAIWorkPlanter extends AbstractEntityAICrafting<JobPlanter, BuildingPlantation>
{
    ...
    private boolean checkIfItemsUnavailable(final IConcreteDeliverable deliverable)
    {
        ...
        if (worksWithoutMaterials() && !deliverable.getRequestedItems().isEmpty())
        {
            // The planter never reaches the shared request chokepoint: it parks the deliverable in currentDeliverable
            // and files it itself in getStateAfterPickUp (line 440 of this file), so it needs gating here instead.
            supplyMaterialWithoutRequest(deliverable.getRequestedItems().get(0), deliverable.getCount() - invCount);
            if (InventoryUtils.getItemCountInItemHandler(worker.getInventoryCitizen(), deliverable::matches) >= deliverable.getMinimumCount())
            {
                return false;
            }
        }
