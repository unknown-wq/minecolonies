...
public class EntityAIArcherTraining extends AbstractEntityAITraining<JobArcherTraining, BuildingArchery>
{
    ...
    protected IAIState shoot()
    {
        ...
        if (isDrawn())
        {
            ...
            final Arrow arrow = ModEntities.MC_NORMAL_ARROW.create(world, EntitySpawnReason.MOB_SUMMONED);
    ...
    /**
     * Whether the shot may be loosed this tick.
     * <p>
     * Normally that is "the citizen is drawing the bow". A hut that forbids tool requests may leave the archer with an
     * empty hand, and vanilla's {@code startUsingItem} silently refuses an empty hand, so the draw would never begin
     * and the state would spin forever. In that case the attack delay alone gates the shot. The arrow is spawned by
     * this AI and carries no damage, so the bow never contributed anything but the animation.
     *
     * @return true if the arrow may be loosed.
     */
    private boolean isDrawn()
    {
        return worker.isUsingItem() || (worksWithoutTools() && worker.getMainHandItem().isEmpty() && currentAttackDelay <= 0);
    }
