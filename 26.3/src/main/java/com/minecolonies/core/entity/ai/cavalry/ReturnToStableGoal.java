...
public class ReturnToStableGoal extends Goal
{
    ...
    {
        // Before the gates, not after: this is the only place a cavalry horse's stable is ever re-resolved, and
        // every gate below stands the goal down while the horse is being ridden. A horse that lost its home - a
        // stable that was torn down and rebuilt, or an older save whose animal data was read before the building
        // manager - therefore stayed homeless for the whole ride, which for a guard's mount is indefinitely, with
        // CavalryStrollGoal and the stall lookup silently doing nothing when it was finally let go. Resolving the
        // home is a data repair; it moves nothing and the check costs one field read once the home is good.
        if (!horse.level().isClientSide() && horse.getAnimalData() != null)
        {
            validateHomeStable();
        }
