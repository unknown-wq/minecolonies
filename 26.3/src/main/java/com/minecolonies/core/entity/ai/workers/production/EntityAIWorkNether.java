...
public class EntityAIWorkNether extends AbstractEntityAICrafting<JobNetherWorker, BuildingNetherWorker>
{
            ...
            for (ItemStack currStack : job.getCraftedResults())
            {
                    ...
                    {
                        {
                            ...
                            LivingEntity mob = (LivingEntity) mobType.create(world, EntitySpawnReason.MOB_SUMMONED);
                            ...
                            for (int hit = 0; mobHealth > 0 && !worker.isDeadOrDying(); hit++)
                            {
                                ...
                                worker.hurtTime = 0;
                                // 26.3: Entity.invulnerableTime is private; the public setter replaces it, so no
                                // accesswidener entry is needed.
                                ...
                                if (!sword.isEmpty())
                                {
                                    if (sword.is(ItemTags.SWORDS))
                                    {
                                        // 26.2 dropped SwordItem#getDamage; the weapon's own attack
                                        // damage now lives in its ATTRIBUTE_MODIFIERS component.
                                        damageToDo += sword.getOrDefault(DataComponents.ATTRIBUTE_MODIFIERS, ItemAttributeModifiers.EMPTY)
                                                        .compute(Attributes.ATTACK_DAMAGE, 0.0D, EquipmentSlot.MAINHAND);
                                    }
                                    else
                                    {
                                    ...
                                    if (doDamage)
                                    {
                                        sword.hurtAndBreak(1, (ServerLevel) worker.level(), null, item -> {
                                ...
                                // Get hit by the mob
                                // 26.2: Entity#hurt(DamageSource, float) returns void; hurtOrSimulate is the boolean-returning form.
                                if (takeDamage && !worker.hurtOrSimulate(source, incomingDamage))
                                {
                            ...
                            else
                            {
                                ...
                                LootTable loot = mob.getLootTable().map(key -> world.getServer().reloadableRegistries().getLootTable(key)).orElse(LootTable.EMPTY);
                ...
                else if (!currStack.isEmpty())
                {
                    ...
                    if (currStack.getItem() instanceof BlockItem bi)
                    {
                        ...
                        if (tool.has(DataComponents.TOOL))
                        {
                            ...
                            {
                                LootParams context = this.getLootContext();
                                LootTable loot = block.getLootTable().map(key -> world.getServer().reloadableRegistries().getLootTable(key)).orElse(LootTable.EMPTY);
                                List<ItemStack> mobLoot = loot.getRandomItems(context);

                                job.addProcessedResultsList(mobLoot);
                                expeditionLog.addLoot(mobLoot);
                                tool.hurtAndBreak(1, (ServerLevel) worker.level(), null, item -> {});
                                if (tool.isEmpty())
                                {
    ...
    private void useFlintAndSteel()
    {
        ...
        tool.hurtAndBreak(1, (ServerLevel) world, null, item -> {});
    }
