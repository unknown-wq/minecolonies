...
public class VisitorCitizen extends AbstractEntityCitizen
{
    ...
    {
        if (!(damageSource.getEntity() instanceof EntityCitizen) && super.hurtServer(serverLevel, damageSource, damage))
        {
    ...
    public void die(DamageSource cause)
    {
        ...
        if (!level().isClientSide())
        {
            ...
            if (colony != null && getCitizenData() != null)
            {
                ...
                // Upstream (1.21.1 and this port both) tested `getHomeBuilding() instanceof TavernBuildingModule`, and a
                // building is never one of its own modules, so the mourning period after a visitor was killed has never
                // once been applied: the tavern simply queued up the next arrival on its ordinary timer. Reach the module
                // the way every other caller does. Matters more now that the interval is a setting - an operator who
                // shortens it is entitled to expect the pause after a death to shorten with it, and it has to exist first.
                final IBuilding deathHome = getCitizenData().getHomeBuilding();
                if (deathHome != null && deathHome.hasModule(BuildingModules.TAVERN_VISITOR))
                {
                    deathHome.getModule(BuildingModules.TAVERN_VISITOR).setNoVisitorTime(level().getRandom().nextInt(5000) + 30000);
                }
