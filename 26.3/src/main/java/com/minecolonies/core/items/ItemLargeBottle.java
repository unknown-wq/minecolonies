...
import com.minecolonies.core.util.PlayerMainInvWrapper;
...
public class ItemLargeBottle extends Item
{
        ...
        @NotNull final InteractionHand hand)
    {
        ...
        if (entity instanceof Cow && !entity.isBaby())
        {
            ...
            {
                player.drop(ModItems.large_milk_bottle.getDefaultInstance(), false, Prediction.PREDICTED);
            }
            stack.shrink(1);
        ...
        if (blockhitresult.getType() != HitResult.Type.MISS)
        {
            if (blockhitresult.getType() == HitResult.Type.BLOCK)
            {
                ...
                if (level.getFluidState(blockpos).is(FluidTags.WATER))
                {
                    ...
                    {
                        player.drop(ModItems.large_water_bottle.getDefaultInstance(), false, Prediction.PREDICTED);
                    }
                    itemstack.shrink(1);
