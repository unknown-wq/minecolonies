...
public class ItemScrollColonyAreaTP extends AbstractItemScroll
{
    ...
    protected ItemStack onItemUseSuccess(final ItemStack itemStack, final Level world, final ServerPlayer player)
    {
        ...
        {
            // Fail chance
            player.sendSystemMessage(Component.translatableEscape(
              "minecolonies.scroll.failed" + (world.getRandom().nextInt(FAIL_RESPONSES_TOTAL) + 1)).setStyle(Style.EMPTY.withColor(
              ChatFormatting.GOLD)));

            itemStack.shrink(1);
            if (!ItemStackUtils.isEmpty(itemStack))
            {
                // 26.3: LivingEntity#drop's third parameter is no longer retainOwnership but a Prediction.
                // PREDICTED is what vanilla passes for a player-initiated drop and keeps the swing animation
                // the old two-argument overload always played. retainOwnership=false was the vanilla default
                // and has no counterpart any more.
                player.drop(itemStack.copy(), true, Prediction.PREDICTED);
    ...
    public void onUseTick(Level worldIn, LivingEntity entity, ItemStack stack, int count)
    {
        if (!worldIn.isClientSide() && worldIn.getGameTime() % 5 == 0 && entity instanceof Player)
        {
            ...
            {
                new VanillaParticleMessage(player.getX(), player.getY(), player.getZ(), SpellParticleOption.create(ParticleTypes.INSTANT_EFFECT, 0xFFFFFF, 1.0F)).sendToTrackingEntity(player);
            }

            new VanillaParticleMessage(sPlayer.getX(), sPlayer.getY(), sPlayer.getZ(), SpellParticleOption.create(ParticleTypes.INSTANT_EFFECT, 0xFFFFFF, 1.0F)).sendToPlayer(sPlayer);
        }
    }
