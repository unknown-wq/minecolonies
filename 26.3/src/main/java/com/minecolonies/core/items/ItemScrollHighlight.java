...
public class ItemScrollHighlight extends AbstractItemScroll
{
    ...
    public InteractionResult useOn(UseOnContext ctx)
    {
        ...
        if (te instanceof TileEntityColonyBuilding)
        {
            ...
            {
                ctx.getPlayer()
                  .sendSystemMessage(Component.translatableEscape(
                    "minecolonies.scroll.failed" + (ctx.getLevel().getRandom().nextInt(FAIL_RESPONSES_TOTAL) + 1)).setStyle(Style.EMPTY.withColor(
                    ChatFormatting.GOLD)));
