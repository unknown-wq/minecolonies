...
public class ItemAssistantHammer extends AbstractItemMinecolonies
{
    ...
    public void useOnBlock(final Player player, final BlockPos interactPos)
    {
        ...
        for (final IWorkOrder workOrder : view.getWorkOrders())
        {
                ...
                && workOrder.getBoundingBox() != null && workOrder.getBoundingBox().inflate(2).contains(Vec3.atLowerCornerOf(interactPos)))
            {
                ...
                {
                    player.sendSystemMessage(Component.translatable("item.minecolonies.assistanthammer.noitems"));
                }
        ...
        if (unclaimed)
        {
            player.sendSystemMessage(Component.translatable("item.minecolonies.assistanthammer.onlyactive"));
        }
    }
    ...
    public void placeBlock(final Player player, final IColony colony, final IWorkOrder workOrder, final BlockPos interactPos)
    {
        if (workOrder.isClaimed())
        {
            ...
            {
                player.sendSystemMessage(Component.translatable("item.minecolonies.assistanthammer.notcleared"));
            ...
            {
                workOrder.loadBlueprint(player.level(), b -> {});
                player.sendSystemMessage(Component.translatable("item.minecolonies.assistanthammer.notloaded"));
                player.inventoryMenu.broadcastFullState();
                return;
            }
            ...
            {
                // 26.2/Fabric: NeoForge's IItemExtension#damageItem (a per-item hook letting an item rewrite
                // the damage amount) has no counterpart -- the plain damage of 1 is applied directly.
    ...
    public void appendHoverText(@NotNull final ItemStack stack,
      ...
      @NotNull final Consumer<Component> tooltipList,
      @NotNull final TooltipFlag flagIn)
    {
        tooltipList.accept(Component.translatable("item.minecolonies.assistanthammer.reach", reach).withStyle(ChatFormatting.BLUE));
        tooltipList.accept(Component.translatable("item.minecolonies.assistanthammer.desc").withStyle(ChatFormatting.ITALIC).withStyle(ChatFormatting.GRAY));
    }
}
