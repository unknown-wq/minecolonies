...
public class ItemScepterPermission extends AbstractItemMinecolonies implements IBlockOverlayItem
{
    ...
    public List<OverlayBox> getOverlayBoxes(@NotNull final Level world, @NotNull final Player player, @NotNull final ItemStack stack)
    {
        ...
        switch (PermissionMode.readFromItemStack(stack))
        {
            case BLOCK:
                ...
                // 26.3: BlockPos.withinManhattan(pos, x, y, z) was renamed. Three variants now differ only in
                // their iteration depth; the one with depth x+y+z -- verified with javap -c against the 26.2
                // jar -- is withinBoxByManhattanDistance.
      ...
      @NotNull final TooltipFlag flags)
    {
        ...
        tooltip.accept(Component.translatable(TOOL_PERMISSION_SCEPTER_MODE, mode.withStyle(ChatFormatting.YELLOW)));
