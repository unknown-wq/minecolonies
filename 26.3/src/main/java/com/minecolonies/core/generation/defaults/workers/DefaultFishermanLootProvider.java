 ...
 */
public class DefaultFishermanLootProvider implements SimpleLootTableProvider.SubProvider
{
    /**
     * The registries, kept only to build loot table references.
     *
     * <p>Port note (26.3): {@code NestedLootTable.lootTableReference} takes a {@code Holder<LootTable>} instead of a
     * {@code ResourceKey<LootTable>} ({@code .../loot/entries/NestedLootTable.java:71}). None of the tables referenced
     * below exist in a registry at datagen time -- three of them are written by this very provider, the other three
     * are vanilla tables -- so the holders are stand-alone references; see
     * {@link SimpleLootTableProvider#tableRef}.</p>
     */
    ...
    private void registerStandardLoot(final BiConsumer<ResourceKey<LootTable>, LootTable.Builder> generator)
    {
        generator.accept(ModLootTables.FISHING, LootTable.lootTable()
                .withPool(LootPool.lootPool()
                        .add(NestedLootTable.lootTableReference(SimpleLootTableProvider.tableRef(provider, ModLootTables.FISHING_JUNK)).setWeight(10).setQuality(-2))
                        .add(NestedLootTable.lootTableReference(SimpleLootTableProvider.tableRef(provider, ModLootTables.FISHING_TREASURE)).setWeight(5).setQuality(2)
                        ...
                        .add(NestedLootTable.lootTableReference(SimpleLootTableProvider.tableRef(provider, ModLootTables.FISHING_FISH)).setWeight(85).setQuality(-1))
                ));

        generator.accept(ModLootTables.FISHING_JUNK, LootTable.lootTable()
                .withPool(LootPool.lootPool()
                        .add(NestedLootTable.lootTableReference(SimpleLootTableProvider.tableRef(provider, BuiltInLootTables.FISHING_JUNK)).setWeight(1))
                ));

        generator.accept(ModLootTables.FISHING_TREASURE, LootTable.lootTable()
                .withPool(LootPool.lootPool()
                        .add(NestedLootTable.lootTableReference(SimpleLootTableProvider.tableRef(provider, BuiltInLootTables.FISHING_TREASURE)).setWeight(1))
                ));

        generator.accept(ModLootTables.FISHING_FISH, LootTable.lootTable()
                .withPool(LootPool.lootPool()
                        .add(NestedLootTable.lootTableReference(SimpleLootTableProvider.tableRef(provider, BuiltInLootTables.FISHING_FISH)).setWeight(1))
                ));
    }
