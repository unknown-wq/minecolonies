...
{
    public DefaultAdvancementsProvider(FabricPackOutput output, CompletableFuture<HolderLookup.Provider> registries)
    {
    ...
    @Override
    public void generateAdvancement(final HolderLookup.Provider registries, final Consumer<AdvancementHolder> consumer)
    {
        ...
        Advancement.Builder.advancement()
            ...
            // 26.3: Advancement.Builder#display lost its background parameter -- a background makes an advancement a
            // tab root, so the two cases are separate methods now: display(...) without one, rootDisplay(...) with one
            // (/opt/mc-src-26.3/net/minecraft/advancements/Advancement.java:145-195). This one passed null.
            // The ItemLike overload went too; only Item and ItemStackTemplate are left.
        ...
        @NotNull final Consumer<AdvancementHolder> consumer)
    {
        ...
        final AdvancementHolder colonyPopulation5 = Advancement.Builder.advancement()
            .parent(buildCitizen)
            .display(make(AdvancementType.GOAL, Items.BED.pick(DyeColor.RED), "colony_population_5"))
    ...
    /**
     * 26.2: {@code ItemPredicate.Builder#of} needs the item lookup, which is only available once the registries
     * future resolves; {@link #generateAdvancement} stashes it here for the static builder helpers.
     */
    private static HolderGetter<Item> items;

    /**
     * 26.3: {@code ItemUsedOnLocationTrigger.TriggerInstance#placedBlock} takes a block lookup as well
     * ({@code /opt/mc-src-26.3/net/minecraft/advancements/triggers/ItemUsedOnLocationTrigger.java:64}), stashed the
     * same way and for the same reason as {@link #items}.
     */
    private static HolderGetter<Block> blocks;

    private static List<ItemPredicate> item(@NotNull final ItemLike item)
    {
        return List.of(ItemPredicate.Builder.item().of(items, item).build());
    }
        ...
        @NotNull final Consumer<AdvancementHolder> consumer)
    {
        ...
        final AdvancementHolder postAndStash = Advancement.Builder.advancement()
            ...
            .addCriterion("postbox", ItemUsedOnLocationTrigger.TriggerInstance.placedBlock(blocks, ModBlocks.blockPostBox))
            .addCriterion("stash", ItemUsedOnLocationTrigger.TriggerInstance.placedBlock(blocks, ModBlocks.blockStash))
