...
public class DefaultNetherWorkerLootProvider extends CustomRecipeAndLootTableProvider
{
    ...
    /**
     * The per-level trip loot, built on first use.
     * <p>
     * Port note (26.2): this used to be built in the constructor. It cannot be any more --
     * {@code FabricDataGenerator.Pack#addProvider} invokes the provider factory while the data generator is being
     * assembled, and the tables here contain {@code ItemStack}s, whose components are not bound that early
     * ({@code NullPointerException: Components not bound yet} out of {@code Holder$Reference#components}). Both
     * consumers below run during generation proper, which is late enough.
     */
    private List<LootTable.Builder> levels = null;
    ...
    /**
     * @return the per-level trip loot tables, built on first use.
     */
    private List<LootTable.Builder> getLevels()
    {
        if (levels == null)
        {
    ...
    private LootPool.Builder createBlocksPool(final int buildingLevel)
    {
        final LootPool.Builder blocks = new LootPool.Builder()
                ...
                .setBonusRolls(ContextFloatProviders.between(0.3F, 0.3F));

        blocks.add(LootItem.lootTableItem(Items.NETHERRACK)
                .setWeight(20)
                .apply(SetItemCountFunction.setCount(ContextIntProviders.between(5, 25))));
    ...
    private LootPool.Builder createMobsPool(final int buildingLevel)
    {
        final LootPool.Builder mobs = new LootPool.Builder()
                ...
                .setBonusRolls(ContextFloatProviders.between(0.1F, 0.1F));
    ...
    private UniformContainerBase.Builder<?> createAdventureToken(@NotNull final EntityType<?> mob, final int damage_done, final int xp_gained)
    {
    ...
    protected void registerRecipes(@NotNull final Consumer<CustomRecipeBuilder> consumer)
    {
        ...
        for (int i = 0; i < getLevels().size(); ++i)
        {
            ...
            final List<LootTableAnalyzer.LootDrop> drops = LootTableAnalyzer.toDrops(provider, Holder.direct(getLevels().get(i).build()));
    ...
    protected List<SimpleLootTableProvider.SubProviderEntry> registerTables()
    {
        return List.of(new SimpleLootTableProvider.SubProviderEntry(provider -> builder ->
        {
            ...
            {
                final int buildingLevel = i + 1;
                builder.accept(table(Identifier.fromNamespaceAndPath(MOD_ID, "recipes/" + NETHERWORKER + "/trip" + buildingLevel)), getLevels().get(i));
            }
