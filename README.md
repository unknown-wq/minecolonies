...
```
dist/minecolonies-26.3-0.0.79.jar          44 MB   this branch's build, for 26.3-snapshot-10
...
this port's — the same lines are in upstream's 1.21.1 sources.
...
in upstream's own 1.21.1 sources before being called upstream's rather than the port's own doing — in
...
gradle build                                   # jar lands in 26.3/build/libs/
```

Run one Gradle invocation at a time: two Loom builds at once race on the same Minecraft cache and the
loser corrupts it.

The dependency jars are taken from the paths in `26.3/gradle.properties`; build `libs/blockui`,
`libs/domum-ornamentum`, `libs/structurize` and `worldmap` the same way first, or point those
properties at jars you already have.
...
Some NeoForge-only hooks have no counterpart in Fabric or in vanilla 26.3. Everything cut was kept
...
| **Citizen minecarts** | Rail physics moved into a package-private `MinecartBehavior` | Cart rides run on vanilla rail speed and curve handling rather than the tuned MineColonies one. The cart still carries only its own citizen and still discards itself once empty |
...
| **Hut item tooltips** | `Block#appendHoverText` is gone; only items have it now | A hut item no longer shows its building level and owning colony in the tooltip |
...
[`26.2/LICENSE`](26.2/LICENSE) and [`26.3/LICENSE`](26.3/LICENSE).
...
asset is stored here**. That tree is absent from every commit:

| Absent | What it was | Why |
|---|---|---|
| `src/main/resources/assets/minecolonies/` in both `26.2/` and `26.3/` | textures, sounds, models, blockstates, GUI, particles, shaders, the language file | governed by its own `LICENSE` file reading **All Rights Reserved** |

The generated asset output is absent with it. **Blueprints are not affected and ship as before**:
`26.2/src/main/resources/blueprints/` carries no licence file of its own and so falls under the
GPLv3 above — the All-Rights-Reserved
